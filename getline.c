...
int getLine(FILE *inputFile, char *commentChar, char *line,
  ...
  if (linePtr == NULL) {
    if (exit_on_EOF) {
      sprintf(messageStr, "Reached end of input file before all data was read");
...
int getLineString(char **inputString, char *commentChar, char *line,
  ...
  while ((linePtr = sgets(line, MAX_LINE_SIZE, inputString)) != NULL) {
    if ((sscanf(line, "%1s", dummy) > 0) &&
        (*(trimwhitespace(line)) != *commentChar))
      break;
  }
  if (linePtr == NULL) {
    if (exit_on_EOF) {
...
char *readWholeFile(const char *filename) { /* Reads file into string */
...
char *sgets(char *str, int num,
            char **input) { /* Works like fgets, but for a string instead of a
                               file object */
  char *next = *input;
  int numread = 0;
...
char *
trimwhitespace(char *str) { /* Gets rid of leading white space from string */
  char *end;

  while (isspace((unsigned char)*str))
    str++;
  if (*str == 0)
    return str; /* Got all spaces? */
