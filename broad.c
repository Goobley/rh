...
void VanderWaals(AtomicLine *line, double *GvdW) {
  ...
  case UNSOLD:
    ...
    vrel35_H = pow(8.0 * KBOLTZMANN / (PI * AMU * atom->weight) *
                       (1.0 + atom->weight / atmos.H->weight),
                   0.3);

    cross = 8.08 *
            (line->cvdWaals[0] * vrel35_H +
             line->cvdWaals[2] * He->abund * vrel35_He) *
            C625;
  ...
  case RIDDER_RENSBERGEN:
    ...
    for (k = 0; k < atmos.Nspace; k++)
      GvdW[k] = gammaCorrH * line->cvdWaals[0] * pow(T[k], line->cvdWaals[1]) +
                gammaCorrHe * line->cvdWaals[2] * pow(T[k], line->cvdWaals[3]) *
                    He->abund;
  ...
  case BARKLEM:
    ...
    for (k = 0; k < atmos.Nspace; k++)
      GvdW[k] =
          line->cvdWaals[0] * pow(atmos.T[k], (1.0 - line->cvdWaals[1]) / 2.0) +
          cross * pow(T[k], 0.3);
...
void Stark(AtomicLine *line, double *GStark) {
  ...
  int Z;
  double C4, C, Cm, m_electron = M_ELECTRON / AMU, cStark23, cStark, neff_u,
                    neff_l, m_atom_avg = AVERAGE_ATOMIC_WEIGHT, vrel, E_Rydberg;
  Atom *atom = line->atom;
  ...
  if (line->cStark < 0.0) {
    ...
    for (ic = line->i + 1;
         ((atom->stage[ic] < atom->stage[line->i] + 1) && (ic < atom->Nlevel));
         ic++)
      ;
