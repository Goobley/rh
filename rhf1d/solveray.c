...
int main(int argc, char *argv[]) {
  ...
  bool_t result, exit_on_EOF, to_obs, initialize, crosscoupling, analyze_output,
      equilibria_only;
  ...
  if (input.StokesMode == FIELD_FREE || input.StokesMode == POLARIZATION_FREE) {
  ...
  if (atmos.moving || input.StokesMode) {
    ...
    if ((atmos.fd_background = open(input.background_File, O_RDONLY, 0)) ==
        -1) {
      sprintf(messageStr, "Unable to open inputfile %s", input.background_File);
      Error(ERROR_LEVEL_2, argv[0], messageStr);
    }
    readBRS();
  }
  ...
  if (Nspect > 0) {
    ...
    Nread = 0;
    while (fscanf(fp_ray, "%d", &wave_index[Nread]) != EOF)
      Nread++;
  ...
  for (n = 0; n < Nspect; n++) {
    if (wave_index[n] < 0 || wave_index[n] >= spectrum.Nspect) {
      sprintf(messageStr,
              "Illegal value of wave_index[n]: %4d\n"
    ...
    result = xdr_vector(&xdrs, (char *)chi, atmos.Nspace, sizeof(double),
  ...
  if (atmos.Stokes || input.backgr_pol) {
    result = xdr_vector(&xdrs, (char *)spectrum.Stokes_Q[0], spectrum.Nspect,
                        sizeof(double), (xdrproc_t)xdr_double);
    result = xdr_vector(&xdrs, (char *)spectrum.Stokes_U[0], spectrum.Nspect,
                        sizeof(double), (xdrproc_t)xdr_double);
    result = xdr_vector(&xdrs, (char *)spectrum.Stokes_V[0], spectrum.Nspect,
                        sizeof(double), (xdrproc_t)xdr_double);
  }
