...
double w3js(double j1, double j2, double j3, double m1, double m2, double m3) {
  ...
  if (fmod(j1, 0.5) || fmod(m1, 0.5) || fmod(j2, 0.5) || fmod(m2, 0.5) ||
      fmod(j3, 0.5) || fmod(m3, 0.5))
    return w3js;
  ...
  for (t = tmin; t <= tmax; t++) {
    sumt +=
        ((t % 2) ? -1.0 : 1.0) / (fact(t) * fact(q3 - t) * fact(q4 - t) *
                                  fact(q5 - t) * fact(q1 + t) * fact(q2 + t));
  }

  w3js = ((((int)(j1 - j2 - m3)) % 2) ? -1.0 : 1.0) *
         sqrt(triangleCoefficient(j1, j2, j3) * fact(j1 + m1) * fact(j1 - m1) *
...
double w6js(double j1, double j2, double j3, double J1, double J2, double J3) {
  ...
  int zmin, zmax;
  double w6js, q1, q2, q3, sum1, sum2, sum3, sum4, tc1, tc2, tc3, tc4, sumz;
  ...
  if (fmod(j1, 0.5) || fmod(J1, 0.5) || fmod(j2, 0.5) || fmod(J2, 0.5) ||
      fmod(j3, 0.5) || fmod(J3, 0.5))
    return w6js;
  ...
  if (fmod(sum1 = j1 + j2 + j3, 1.0))
    return w6js;
  if (fmod(sum2 = j1 + J2 + J3, 1.0))
    return w6js;
  if (fmod(sum3 = J1 + j2 + J3, 1.0))
    return w6js;
  if (fmod(sum4 = J1 + J2 + j3, 1.0))
    return w6js;
