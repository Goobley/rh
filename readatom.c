...
void readAtom(Atom *atom, bool_t active) {
  ...
  int i, j, Nlevel, Nrad, Nline, Ncont, Nfixed,
      Nspace = atmos.Nspace, Nread, Nrequired, checkPoint, L, nq, status;
  double f, C, lambda0, lambdamin, vtherm, S, Ju, Jl, c_sum, waveratio,
      lambda_air;
  ...
  */
  ...
  if (!match) {
    sprintf(messageStr,
            " No matching element in periodic table for "
  ...
  getLineString(&atom_string, COMMENT_CHAR, inputLine, exit_on_EOF = TRUE);
  Nread = sscanf(inputLine, "%d %d %d %d", &atom->Nlevel, &atom->Nline,
                 &atom->Ncont, &atom->Nfixed);
  ...
  for (i = 0; i < Nlevel; i++) {
    ...
    getLineString(&atom_string, COMMENT_CHAR, inputLine, exit_on_EOF = TRUE);
    Nread = sscanf(inputLine, "%lf %lf '%20c' %d", &atom->E[i], &atom->g[i],
                   atom->label[i], &atom->stage[i]);
  ...
  if (atom->active && (input.isum < -1 || input.isum >= Nlevel)) {
    sprintf(messageStr, "Invalid value for ISUM = %d, Nlevel = %d", input.isum,
            Nlevel);
    Error(ERROR_LEVEL_2, routineName, messageStr);
  }
  ...
  for (kr = 0; kr < Nline; kr++) {
    ...
    Nread = sscanf(inputLine,
                   ...
                   &j, &i, &f, shapeStr, &line->Nlambda, symmStr, &line->qcore,
                   &line->qwing, vdWstr, &line->cvdWaals[0], &line->cvdWaals[1],
                   &line->cvdWaals[2], &line->cvdWaals[3], &line->Grad,
                   &line->cStark, &line->g_Lande_eff);
    ...
    if (!strstr(shapeStr, "PRD") && !strstr(shapeStr, "VOIGT") &&
        !strstr(shapeStr, "GAUSS") && !strstr(shapeStr, "COMPOSIT")) {
      sprintf(messageStr, "Invalid value for line-shape string: %s", shapeStr);
      Error(ERROR_LEVEL_2, routineName, messageStr);
    }
    ...
    if (strstr(shapeStr, "COMPOSIT")) {
      ...
      line->c_fraction = (double *)malloc(line->Ncomponent * sizeof(double));

      c_sum = 0.0;
      for (n = 0; n < line->Ncomponent; n++) {
        getLineString(&atom_string, COMMENT_CHAR, inputLine,
                      exit_on_EOF = TRUE);
        Nread = sscanf(inputLine, "%lf %lf", &line->c_shift[n],
                       &line->c_fraction[n]);
        c_sum += line->c_fraction[n];
      }
      if (c_sum > (1.0 + MAX_ABUND_ERROR) || c_sum < (1.0 - MAX_ABUND_ERROR)) {
        ...
        vacuum_to_air(1, &line->lambda0, &lambda_air);
        sprintf(messageStr, " Line %d -> %d (%9.3f [nm]) has %d components\n",
    ...
    if (strstr(vdWstr, "PARAMTR"))
      ...
      if (!getBarklemactivecross(line)) {
        sprintf(messageStr,
                ...
                "with Barklem type broadening. Using UNSOLD.",
                j, i);
  ...
  atom->continuum = (AtomicContinuum *)malloc(Ncont * sizeof(AtomicContinuum));
  for (kr = 0; kr < Ncont; kr++) {
    ...
    Nread = sscanf(inputLine, "%d %d %lf %d %s %lf", &j, &i, &continuum->alpha0,
                   &continuum->Nlambda, nuDepStr, &lambdamin);
    ...
    if (strstr(nuDepStr, "EXPLICIT")) {
      ...
      for (la = continuum->Nlambda - 1; la >= 0; la--) {
        getLineString(&atom_string, COMMENT_CHAR, inputLine,
                      exit_on_EOF = TRUE);
        Nread = sscanf(inputLine, "%lf %lf", &continuum->lambda[la],
                       &continuum->alpha[la]);
      ...
      if (lambdamin >= continuum->lambda0) {
        sprintf(messageStr,
                "Minimum wavelength for continuum %d -> %d"
      ...
      sprintf(messageStr, "Invalid value for wavelength dependence string: %s",
              nuDepStr);
      Error(ERROR_LEVEL_2, routineName, messageStr);
    }
    if (atom->active) {
  ...
  if (atom->Nfixed > 0) {
    ...
    for (kf = 0; kf < Nfixed; kf++) {
      ...
      getLineString(&atom_string, COMMENT_CHAR, inputLine, exit_on_EOF = TRUE);
      Nread = sscanf(inputLine, "%d %d %lf %lf %s", &j, &i, &fixed->strength,
      ...
      for (kr = 0; kr < Nline; kr++) {
        ...
        if (line->i == i && line->j == j) {
          sprintf(messageStr,
                  "Fixed transition j = %d, i = %d duplicates active line", j,
                  i);
          Error(ERROR_LEVEL_2, routineName, messageStr);
        }
      }
      for (kr = 0; kr < Ncont; kr++) {
        ...
        if (continuum->i == i && continuum->j == j) {
          sprintf(messageStr,
                  "Fixed transition j = %d,  i = %d"
                  " duplicates active continuum",
                  j, i);
          Error(ERROR_LEVEL_2, routineName, messageStr);
        }
      }
      ...
      if (strstr(optionStr, "TRAD_ATMOSPHERIC"))
        ...
        sprintf(messageStr,
                "Invalid value for radiation temperature"
                " option string: %s",
                optionStr);
        Error(ERROR_LEVEL_2, routineName, messageStr);
      }
  ...
  if (atom->active) {
    ...
    sprintf(atom->popsoutFile,
            (atom->ID[1] == ' ') ? "pops.%.1s.out" : "pops.%.2s.out", atom->ID);
        ...
        for (kr = 0; kr < Nline; kr++) {
          ...
          if (line->PRD) {
            for (krp = 0; krp < Nline; krp++) {
              if (atom->line[krp].PRD && line->j == atom->line[krp].j &&
            ...
            line->xrd = realloc(line->xrd, line->Nxrd * sizeof(AtomicLine *));
    ...
    if (atom->popsinFile && atom->initial_solution == OLD_POPULATIONS) {
  ...
  sprintf(labelStr, "Read %s %2s", (atom->active) ? "Active" : "Atom",
          atom->ID);
...
void freeAtom(Atom *atom) {
  ...
  if (atom->popsinFile != NULL)
    free(atom->popsinFile);
  if (atom->popsoutFile != NULL)
    free(atom->popsoutFile);
  if (atom->stage != NULL)
    free(atom->stage);
  if (atom->g != NULL)
    free(atom->g);
...
void freeAtomicLine(AtomicLine *line) {
  ...
  if (line->c_shift != NULL)
    free(line->c_shift);
  if (line->c_fraction != NULL)
...
void freeAtomicContinuum(AtomicContinuum *continuum) {
  ...
  if (continuum->lambda != NULL)
    free(continuum->lambda);
  if (continuum->Rij != NULL)
    free(continuum->Rij);
  if (continuum->Rji != NULL)
    free(continuum->Rji);

  if (continuum->alpha != NULL)
...
void readAtomicModels(void) {
  ...
  char filename[MAX_LINE_SIZE], actionKey[MAX_KEYWORD_SIZE],
      popsKey[MAX_KEYWORD_SIZE], popsFile[MAX_LINE_SIZE],
      inputLine[MAX_LINE_SIZE], *atomID;
  ...
  for (n = 0; n < atmos.Natom; n++) {
    ...
    getLineString(&fp_atoms, COMMENT_CHAR, inputLine, exit_on_EOF = TRUE);
    Nread = sscanf(inputLine, "%s %s %s %s ", filename, actionKey, popsKey,
                   popsFile);
    checkNread(Nread, Nrequired = 3, routineName, checkPoint = 2);

    atomID = getAtomID(filename);
    if (n == 0 && !strstr(atomID, "H ")) {
      sprintf(messageStr, "First atomic model is not hydrogen: %s", atomID);
      Error(ERROR_LEVEL_2, routineName, messageStr);
    }
    ...
    for (m = 0; m < n; m++) {
      if (strstr(atomID, atmos.atoms[m].ID)) {
        sprintf(messageStr, "Already read atomic model for element %s\n",
                atomID);
        Error(ERROR_LEVEL_2, routineName, messageStr);
      }
    }
    ...
    if (atom->initial_solution == UNKNOWN) {
      sprintf(messageStr, "Unknown initial solution specified for atom: %s\n",
              atomID);
      Error(ERROR_LEVEL_2, routineName, messageStr);
    }
    ...
    if (atom->initial_solution == OLD_POPULATIONS) {
      ...
      atom->popsinFile = (char *)malloc((strlen(popsFile) + 1) * sizeof(char));
