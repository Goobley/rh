...
void ChemicalEquilibrium(int NmaxIter, double iterLimit) {
  ...
  char tmpStr[7];
  bool_t quiet;
  int Nequation, **nucl_index, niter, Nnuclei, Ngdelay, Ngperiod, Ngorder,
      Nmaxstage;
  double *f, *a, *n, **df, *Phi, PhiHmin, fHmin, CI,
      dnmax = 0.0, *fn0, fraction, saha, *fjk, *dfjk;
  struct Ng *Ngn;
  Atom *atom;
  Molecule *molecule;
  Element **nuclei;
  ...
  Ngn = NgInit(Nequation, Ngdelay = NG_CHEM_DELAY, Ngorder = NG_CHEM_ORDER,
               Ngperiod = NG_CHEM_PERIOD, n);
    ...
    for (i = 0; i < Nnuclei; i++) {
      if ((atom = nuclei[i]->model) == NULL) {
        ...
        for (j = 0; j < atom->Nlevel; j++) {
          if (atom->stage[j] > 0)
            break;
          if (atom->active && atom->initial_solution != OLD_POPULATIONS)
  ...
  for (nu = 0; nu < Nnuclei; nu++) {
    ...
    if (atom && atom->active) {
      ...
      for (i = 0; i < nuclei[nu]->Nmolecule; i++) {
        sprintf(tmpStr, "%s%s", atmos.molecules[nuclei[nu]->mol_index[i]].ID,
