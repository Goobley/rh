...
void Solve_ne(double *ne, bool_t fromscratch) {
  ...
  int Nmaxstage, niter;
  double *fjk, *dfjk, error, ne_old, akj, sum, PhiH, C1, Uk, dne, dnemax, *np,
      PhiHmin;
  ...
  for (k = 0; k < atmos.Nspace; k++) {
    ...
    while (niter < N_MAX_ELECTRON_ITERATIONS) {
      ...
      ne[k] = ne_old - atmos.nHtot[k] * error / (1.0 - atmos.nHtot[k] * sum);
    ...
    if (dne > MAX_ELECTRON_ERROR) {
      sprintf(messageStr,
              "Electron density iteration not converged:\n"
...
double getKuruczpf(Element *element, int stage, int k) {
  ...
  Linear(atmos.Npf, atmos.Tpf, element->pf[stage], 1, &atmos.T[k], &Uk, hunt);
