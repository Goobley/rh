...
static double t[NT] = {0.2453407083, 0.7374737285, 1.2340762153, 1.7385377121,
                       2.2549740020, 2.7888060584, 3.3478545673, 3.9447640401,
                       4.6036824495, 5.3874808900};

static double w[NT] = {4.6224366960e-01, 2.8667550536e-01, 1.0901720602e-01,
...
double VoigtK1(double a, double v) {
  static double c[NC] = {0.1999999999972224,
                         -0.1840000000029998,
                         0.1558399999965025,
                         -0.1216640000043988,
                         0.0877081599940391,
                         ...
                         -0.0208497654398036,
                         0.0111960116346270,
                         -0.56231896167109e-02,
                         0.26487634172265e-02,
                         -0.11732670757704e-02,
                         0.4899519978088e-03,
                         -0.1933630801528e-03,
                         0.722877446788e-04,
                         -0.256555124979e-04,
  ...
  double a2 = a * a, v2 = v * v, u1, dn01, dn02, dn, v2i, funct, an, q, g, coef,
         bn01, bn02, bn, v1;
...
double VoigtK2(double a, double v) {
  ...
  for (n = 0; n < NT; n++) {
    ...
    s = t[n] + v;
    g += (4.0 * t[n] * t[n] - 2.0) *
         (r * atan(r / a) + s * atan(s / a) -
          0.5 * a * (log(a2 + r * r) + log(a2 + s * s))) *
         w[n];
  }
  return g / PI;
}
...
double VoigtHui(double a, double v, double *F) {
  ...
  static double ah[NHUI + 1] = {122.607931777104326, 214.382388694706425,
                                181.928533092181549, 93.155580458138441,
                                30.180142196210589,  5.912626209773153,
                                0.564189583562615};

  static double bh[NHUI + 1] = {122.607931773875350, 352.730625110963558,
...
double VoigtLookup(double a, double v) {
  ...
  static double *a_table, *v_table_lin, *v_table_log, **table_lin, **table_log,
      voigt, log_a;
  ...
  if (a < A_MIN || a > A_MAX || v > V_MAX_LOG) {
    sprintf(messageStr, "Arguments (%E, %E) outside domain table", a, v);
  ...
  if (initialize) {
    ...
    for (n = 0; n < N_A; n++) {
      for (m = 0; m < N_V_LIN; m++) {
        table_lin[n][m] =
            Voigt(exp(a_table[n]), v_table_lin[m], NULL, TABLE_ALGORITHM);
      }
      for (m = 0; m < N_V_LOG; m++) {
        table_log[n][m] =
            Voigt(exp(a_table[n]), exp(v_table_log[m]), NULL, TABLE_ALGORITHM);
      }
    }
  ...
  if (v < V_MAX_LIN) {
    voigt = BiLinear(N_A, a_table, log_a, N_V_LIN, v_table_lin, v, table_lin,
                     hunt = TRUE);
  } else {
