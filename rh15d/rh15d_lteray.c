...
int main(int argc, char *argv[]) {
  ...
  for (mpi.task = 0; mpi.task < mpi.Ntasks; mpi.task++) {
    ...
    /* Printout some info */
    sprintf(messageStr,
            "Process %4d: --- START task %3ld [of %ld], "
            "(xi,yi) = (%3d,%3d)\n",
            mpi.rank, mpi.task + 1, mpi.Ntasks, mpi.xnum[mpi.ix],
            mpi.ynum[mpi.iy]);
    ...
    sprintf(messageStr, "Process %4d: *** END   task %3ld\n", mpi.rank,
            mpi.task + 1);
    fprintf(mpi.main_logfile, "%s", messageStr);
    Error(MESSAGE, "main", messageStr);
  ...
  finish_jobs();
  sprintf(messageStr,
          "*** Job ending. Total %ld 1-D columns: %ld computed, "
          "%ld skipped.\n%s",
