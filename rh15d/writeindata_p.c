...
/* Creates the file for the input data */
{
  ...
  /* Create the file  */
  if ((plist = H5Pcreate(H5P_FILE_ACCESS)) < 0)
    HERR(routineName);
  if ((H5Pset_fapl_mpio(plist, mpi.comm, mpi.info)) < 0)
    HERR(routineName);
  if ((ncid = H5Fcreate(INPUTDATA_FILE, H5F_ACC_TRUNC, H5P_DEFAULT, plist)) < 0)
    HERR(routineName);
  if ((H5Pclose(plist)) < 0)
    HERR(routineName);
  ...
  if ((ncid_atmos = H5Gcreate(ncid, "/atmos", H5P_DEFAULT, H5P_DEFAULT,
    ...
    HERR(routineName);
  if ((ncid_mpi =
           H5Gcreate(ncid, "/mpi", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
    HERR(routineName);
  ...
  if ((H5LTmake_dataset(ncid, ZOUT_NAME, 1, dims, H5T_NATIVE_DOUBLE,
    ...
    HERR(routineName);
  free(tmp_double);
  if ((id_z = H5Dopen2(ncid, ZOUT_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  /* For compatibility with netCDF readers, only use dataset as dimension */
  if ((H5LTset_attribute_string(ncid, ZOUT_NAME, "NAME", NETCDF_COMPAT)) < 0)
    HERR(routineName);
  ...
  if ((H5LTset_attribute_uchar(ncid_input, ".", "Magneto_optical",
                               (unsigned char *)&input.magneto_optical, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_uchar(ncid_input, ".", "PRD_angle_dep",
                               (unsigned char *)&PRD_angle_dep, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_uchar(ncid_input, ".", "XRD", (unsigned char *)&XRD,
                               1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_uchar(ncid_input, ".", "Background_polarization",
                               (unsigned char *)&input.backgr_pol, 1)) < 0)
    HERR(routineName);
  ...
  if ((H5LTset_attribute_string(ncid_input, ".", "Stokes_mode", StokesMode)) <
      0)
    HERR(routineName);
  ...
  if ((H5LTset_attribute_string(ncid_input, ".", "Angle_set", angleSet)) < 0)
    HERR(routineName);
  ...
  for (i = 0; i < atmos.Natom; i++) {
    ...
    if ((ncid_tmp = H5Gcreate(ncid_input, group_name, H5P_DEFAULT, H5P_DEFAULT,
                              H5P_DEFAULT)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid_tmp, ".", "file_name",
                                  atom->atom_file)) < 0)
      HERR(routineName);
    if ((H5LTmake_dataset_string(ncid_tmp, "file_contents", atom->fp_input)) <
        0)
      HERR(routineName);
    if ((H5Gclose(ncid_tmp)) < 0)
      HERR(routineName);
  }
  dims[0] = atmos.Natom;
  dims[1] = atom_name_size;
  if ((H5LTmake_dataset(ncid_input, "atom_groups", 2, dims, H5T_C_S1,
                        atom_names[0])) < 0)
    HERR(routineName);
  freeMatrix((void **)atom_names);
  /* Information from ray.input */
  if ((H5LTset_attribute_double(ncid_input, ".", INPUT_MU, &io.ray_muz, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_uint(ncid_input, ".", WAVE_SEL,
                              (unsigned int *)&io.ray_nwave_sel, 1)) < 0)
    HERR(routineName);
  if (io.ray_nwave_sel > 0) {
    dims[0] = io.ray_nwave_sel;
    if ((H5LTmake_dataset(ncid_input, WAVE_SEL_IDX, 1, dims, H5T_NATIVE_UINT,
                          io.ray_wave_idx)) < 0)
      HERR(routineName);
  }
  ...
  if (input.Nkurucz_files > 0) {
    ...
    for (i = 0; i < input.Nkurucz_files; i++) {
      ...
      if ((ncid_tmp = H5Gcreate(ncid_input, group_name, H5P_DEFAULT,
        ...
        HERR(routineName);
      if ((H5LTset_attribute_string(ncid_tmp, ".", "file_name",
                                    input.kurucz_line_file_name[i])) < 0)
        HERR(routineName);
      if ((H5LTmake_dataset_string(ncid_tmp, "file_contents",
                                   input.kurucz_line_file_contents[i])) < 0)
        HERR(routineName);
      if ((H5Gclose(ncid_tmp)) < 0)
        HERR(routineName);
    }
  ...
  /* dimensions */
  if ((H5LTset_attribute_int(ncid_atmos, ".", "nelements", &atmos.Nelem, 1)) <
      0)
    HERR(routineName);
  if ((H5LTset_attribute_int(ncid_atmos, ".", "nrays", &geometry.Nrays, 1)) < 0)
    HERR(routineName);
  /* --- dimension datasets --- */
  dims[0] = mpi.nx;
  if ((H5LTmake_dataset(ncid_atmos, X_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                        geometry.xscale)) < 0)
    HERR(routineName);
  if ((id_x = H5Dopen2(ncid_atmos, X_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  dims[0] = mpi.ny;
  if ((H5LTmake_dataset(ncid_atmos, Y_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                        geometry.yscale)) < 0)
    HERR(routineName);
  if ((id_y = H5Dopen2(ncid_atmos, Y_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  dims[0] = atmos.Nelem;
  tmp = (unsigned int *)calloc(atmos.Nelem, sizeof(unsigned int));
  if ((H5LTmake_dataset(ncid_atmos, ELEM_NAME, 1, dims, H5T_NATIVE_UINT, tmp)) <
      0)
    HERR(routineName);
  free(tmp);
  dims[0] = geometry.Nrays;
  tmp = (unsigned int *)calloc(geometry.Nrays, sizeof(unsigned int));
  if ((H5LTmake_dataset(ncid_atmos, RAY_NAME, 1, dims, H5T_NATIVE_UINT, tmp)) <
      0)
    HERR(routineName);
  free(tmp);
  /* For compatibility with netCDF readers, only use dataset as dimension */
  if ((H5LTset_attribute_string(ncid_atmos, ELEM_NAME, "NAME", NETCDF_COMPAT)) <
      0)
    HERR(routineName);
  if ((H5LTset_attribute_string(ncid_atmos, RAY_NAME, "NAME", NETCDF_COMPAT)) <
      0)
    HERR(routineName);
  ...
  dims[1] = mpi.ny;
  dims[2] = infile.nz;
  if ((file_dspace = H5Screate_simple(3, dims, NULL)) < 0)
    HERR(routineName);
  ...
  if ((H5Pset_alloc_time(plist, H5D_ALLOC_TIME_EARLY)) < 0)
    HERR(routineName);
  if ((H5Pset_fill_time(plist, H5D_FILL_TIME_ALLOC)) < 0)
    HERR(routineName);
  if ((io.in_atmos_T =
           H5Dcreate(ncid_atmos, TEMP_NAME, H5T_NATIVE_FLOAT, file_dspace,
                     H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_T, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_T, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_T, id_z, 2)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_atmos, TEMP_NAME, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((io.in_atmos_vz =
           H5Dcreate(ncid_atmos, VZ_NAME, H5T_NATIVE_FLOAT, file_dspace,
                     H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_vz, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_vz, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_vz, id_z, 2)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_atmos, VZ_NAME, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((io.in_atmos_z =
           H5Dcreate(ncid_atmos, ZH_NAME, H5T_NATIVE_FLOAT, file_dspace,
                     H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_z, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_z, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_z, id_z, 2)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_atmos, ZH_NAME, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((io.in_atmos_ne =
           H5Dcreate(ncid_atmos, NE_NAME, H5T_NATIVE_DOUBLE, file_dspace,
                     H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_ne, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_ne, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_atmos_ne, id_z, 2)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_atmos, NE_NAME, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  ...
  dims[0] = atmos.Nelem;
  if ((id_n = H5Dopen2(ncid_atmos, ELEM_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5LTmake_dataset(ncid_atmos, "element_weight", 1, dims,
                        H5T_NATIVE_DOUBLE, eweight)) < 0)
    HERR(routineName);
  if ((id_tmp = H5Dopen2(ncid_atmos, "element_weight", H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(id_tmp, id_n, 0)) < 0)
    HERR(routineName);
  if ((H5Dclose(id_tmp)) < 0)
    HERR(routineName);
  if ((H5LTmake_dataset(ncid_atmos, "element_abundance", 1, dims,
                        H5T_NATIVE_DOUBLE, eabund)) < 0)
    HERR(routineName);
  if ((id_tmp = H5Dopen2(ncid_atmos, "element_abundance", H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(id_tmp, id_n, 0)) < 0)
    HERR(routineName);
  if ((H5Dclose(id_tmp)) < 0)
  ...
  */
  ...
  dims[0] = geometry.Nrays;
  if ((id_n = H5Dopen2(ncid_atmos, RAY_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5LTmake_dataset(ncid_atmos, "muz", 1, dims, H5T_NATIVE_DOUBLE,
                        geometry.muz)) < 0)
    HERR(routineName);
  if ((id_tmp = H5Dopen2(ncid_atmos, "muz", H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(id_tmp, id_n, 0)) < 0)
    HERR(routineName);
  if ((H5Dclose(id_tmp)) < 0)
    HERR(routineName);
  if ((H5LTmake_dataset(ncid_atmos, "wmu", 1, dims, H5T_NATIVE_DOUBLE,
                        geometry.wmu)) < 0)
    HERR(routineName);
  if ((id_tmp = H5Dopen2(ncid_atmos, "wmu", H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(id_tmp, id_n, 0)) < 0)
    HERR(routineName);
  if ((H5Dclose(id_tmp)) < 0)
  ...
  if ((H5LTset_attribute_uchar(ncid_atmos, ".", "moving",
                               (unsigned char *)&atmos.moving, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_uchar(ncid_atmos, ".", "stokes",
                               (unsigned char *)&atmos.Stokes, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_string(ncid_atmos, TEMP_NAME, "units", UNIT_TEMP)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_string(ncid_atmos, VZ_NAME, "units", UNIT_VELOCITY)) <
      0)
    HERR(routineName);
  if ((H5LTset_attribute_string(ncid_atmos, ZH_NAME, "units", UNIT_LENGTH)) < 0)
    HERR(routineName);
  ...
  /* dimensions */
  if ((H5LTset_attribute_int(ncid_mpi, ".", "nprocesses", &mpi.size, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_int(ncid_mpi, ".", "niterations", &NMaxIter, 1)) < 0)
    HERR(routineName);
  /* --- dimension datasets --- */
  dims[0] = mpi.nx;
  if ((H5LTmake_dataset(ncid_mpi, X_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                        geometry.xscale)) < 0)
    HERR(routineName);
  if ((id_x = H5Dopen2(ncid_mpi, X_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  dims[0] = mpi.ny;
  if ((H5LTmake_dataset(ncid_mpi, Y_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                        geometry.yscale)) < 0)
    HERR(routineName);
  if ((id_y = H5Dopen2(ncid_mpi, Y_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  dims[0] = NMaxIter;
  tmp = (unsigned int *)calloc(NMaxIter, sizeof(unsigned int));
  if ((H5LTmake_dataset(ncid_mpi, IT_NAME, 1, dims, H5T_NATIVE_UINT, tmp)) < 0)
    HERR(routineName);
  free(tmp);
  /* For compatibility with netCDF readers, only use dataset as dimension */
  if ((H5LTset_attribute_string(ncid_mpi, IT_NAME, "NAME", NETCDF_COMPAT)) < 0)
    HERR(routineName);
  /* variables*/
  dims[0] = mpi.nx;
  if ((H5LTmake_dataset(ncid_mpi, XNUM_NAME, 1, dims, H5T_NATIVE_INT,
                        mpi.xnum)) < 0)
    HERR(routineName);
  if ((id_tmp = H5Dopen2(ncid_mpi, XNUM_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(id_tmp, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5Dclose(id_tmp)) < 0)
    HERR(routineName);
  dims[0] = mpi.ny;
  if ((H5LTmake_dataset(ncid_mpi, YNUM_NAME, 1, dims, H5T_NATIVE_INT,
                        mpi.ynum)) < 0)
    HERR(routineName);
  if ((id_tmp = H5Dopen2(ncid_mpi, YNUM_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(id_tmp, id_y, 0)) < 0)
    HERR(routineName);
  if ((H5Dclose(id_tmp)) < 0)
    HERR(routineName);
  dims[0] = mpi.nx;
  dims[1] = mpi.ny;
  if ((file_dspace = H5Screate_simple(2, dims, NULL)) < 0)
    HERR(routineName);
  if ((plist = H5Pcreate(H5P_DATASET_CREATE)) < 0)
    HERR(routineName);
  if ((H5Pset_fill_value(plist, H5T_NATIVE_FLOAT, &FILLVALUE)) < 0)
  ...
  if ((io.in_mpi_tm = H5Dcreate(ncid_mpi, TASK_MAP, H5T_NATIVE_LONG,
                                file_dspace, H5P_DEFAULT, plist, H5P_DEFAULT)) <
      0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_tm, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_tm, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_mpi, TASK_MAP, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((io.in_mpi_tn = H5Dcreate(ncid_mpi, TASK_NUMBER, H5T_NATIVE_LONG,
      ...
      0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_tn, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_tn, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_mpi, TASK_NUMBER, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((io.in_mpi_it = H5Dcreate(ncid_mpi, ITER_NAME, H5T_NATIVE_LONG,
      ...
      0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_it, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_it, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_mpi, ITER_NAME, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((io.in_mpi_conv =
           H5Dcreate(ncid_mpi, CONV_NAME, H5T_NATIVE_LONG, file_dspace,
                     H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_conv, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_conv, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_mpi, CONV_NAME, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((io.in_mpi_dm = H5Dcreate(ncid_mpi, DM_NAME, H5T_NATIVE_FLOAT,
      ...
      0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_dm, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_dm, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_mpi, DM_NAME, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((io.in_mpi_zc = H5Dcreate(ncid_mpi, ZC_NAME, H5T_NATIVE_INT, file_dspace,
                                H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_zc, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_zc, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_mpi, ZC_NAME, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((H5Pclose(plist)) < 0)
    HERR(routineName);
  ...
  if ((H5Pset_fill_time(plist, H5D_FILL_TIME_ALLOC)) < 0)
    HERR(routineName);
  if ((io.in_mpi_dmh =
           H5Dcreate(ncid_mpi, DMH_NAME, H5T_NATIVE_FLOAT, file_dspace,
                     H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((id_tmp = H5Dopen2(ncid_mpi, IT_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_dmh, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_dmh, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.in_mpi_dmh, id_tmp, 2)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid_mpi, DMH_NAME, "_FillValue", &FILLVALUE,
                               1)) < 0)
    HERR(routineName);
  if ((H5Dclose(id_tmp)) < 0)
    HERR(routineName);
  ...
  /* attributes */
  if ((H5LTset_attribute_int(ncid_mpi, ".", "x_start", &input.p15d_x0, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_int(ncid_mpi, ".", "x_end", &input.p15d_x1, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_int(ncid_mpi, ".", "x_step", &input.p15d_xst, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_int(ncid_mpi, ".", "y_start", &input.p15d_y0, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_int(ncid_mpi, ".", "y_end", &input.p15d_y1, 1)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_int(ncid_mpi, ".", "y_step", &input.p15d_yst, 1)) < 0)
    HERR(routineName);
  ...
  */
  ...
  if ((H5Fflush(ncid, H5F_SCOPE_LOCAL)) < 0)
    HERR(routineName);
...
/* Opens an existing input data file, loads structures and ids */
{
  ...
  if ((io.in_atmos_T = H5Dopen(io.in_atmos_ncid, TEMP_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((io.in_atmos_vz = H5Dopen(io.in_atmos_ncid, VZ_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((io.in_atmos_z = H5Dopen(io.in_atmos_ncid, ZH_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((io.in_atmos_ne = H5Dopen(io.in_atmos_ncid, NE_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);

  if ((io.in_mpi_tm = H5Dopen(io.in_mpi_ncid, TASK_MAP, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((io.in_mpi_tn = H5Dopen(io.in_mpi_ncid, TASK_NUMBER, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((io.in_mpi_it = H5Dopen(io.in_mpi_ncid, ITER_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((io.in_mpi_conv = H5Dopen(io.in_mpi_ncid, CONV_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((io.in_mpi_dm = H5Dopen(io.in_mpi_ncid, DM_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((io.in_mpi_dmh = H5Dopen(io.in_mpi_ncid, DMH_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if ((io.in_mpi_zc = H5Dopen(io.in_mpi_ncid, ZC_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if (input.wavetable != NULL)
    free(input.wavetable);
  return;
...
/* Closes the indata file */
{
  ...
  /* Close all datasets */
  if ((H5Dclose(io.in_atmos_T)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_atmos_vz)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_atmos_z)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_atmos_ne)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_mpi_tm)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_mpi_tn)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_mpi_it)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_mpi_conv)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_mpi_dm)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_mpi_dmh)) < 0)
    HERR(routineName);
  if ((H5Dclose(io.in_mpi_zc)) < 0)
    HERR(routineName);

  /* Close all groups */
  if ((H5Gclose(io.in_input_ncid)) < 0)
    HERR(routineName);
  if ((H5Gclose(io.in_atmos_ncid)) < 0)
    HERR(routineName);
  if ((H5Gclose(io.in_mpi_ncid)) < 0)
    HERR(routineName);

  /* Close file */
  if ((H5Fclose(io.in_ncid)) < 0)
    HERR(routineName);

  /* Free other resources */
  if (input.keyword_file_contents != NULL)
    free(input.keyword_file_contents);
...
void writeAtmos_p(void) {
  ...
  if ((file_dspace = H5Dget_space(io.in_atmos_T)) < 0)
    HERR(routineName);
  if ((H5Sselect_hyperslab(file_dspace, H5S_SELECT_SET, offset, NULL, count,
                           NULL)) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_atmos_T, H5T_NATIVE_DOUBLE, mem_dspace, file_dspace,
                H5P_DEFAULT, atmos.T)) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_atmos_vz, H5T_NATIVE_DOUBLE, mem_dspace, file_dspace,
                H5P_DEFAULT, geometry.vel)) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_atmos_z, H5T_NATIVE_DOUBLE, mem_dspace, file_dspace,
                H5P_DEFAULT, geometry.height)) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_atmos_ne, H5T_NATIVE_DOUBLE, mem_dspace, file_dspace,
                H5P_DEFAULT, atmos.ne)) < 0)
    HERR(routineName);
  /* release dataspace resources */
  if ((H5Sclose(mem_dspace)) < 0)
    HERR(routineName);
  if ((H5Sclose(file_dspace)) < 0)
    HERR(routineName);
  return;
...
void writeMPI_all(void) {
  ...
  for (task = 0; task < mpi.Ntasks; task++) {
    ...
    if ((file_dspace = H5Dget_space(io.in_mpi_dmh)) < 0)
      HERR(routineName);
    if ((H5Sselect_hyperslab(file_dspace, H5S_SELECT_SET, offset, NULL, count,
                             NULL)) < 0)
      HERR(routineName);
    if ((H5Dwrite(io.in_mpi_dmh, H5T_NATIVE_DOUBLE, mem_dspace, file_dspace,
                  H5P_DEFAULT, mpi.dpopsmax_hist[task])) < 0)
      HERR(routineName);
    if ((H5Sclose(file_dspace)) < 0)
      HERR(routineName);
    if ((H5Sclose(mem_dspace)) < 0)
...
void writeMPI_p(int task) {
  ...
  offset[0] = mpi.ix;
  offset[1] = mpi.iy;
  if ((file_dspace = H5Dget_space(io.in_mpi_tm)) < 0)
    HERR(routineName);
  if ((H5Sselect_hyperslab(file_dspace, H5S_SELECT_SET, offset, NULL, count,
                           NULL)) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_mpi_tm, H5T_NATIVE_INT, mem_dspace, file_dspace,
                H5P_DEFAULT, &mpi.rank)) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_mpi_tn, H5T_NATIVE_INT, mem_dspace, file_dspace,
                H5P_DEFAULT, &task)) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_mpi_it, H5T_NATIVE_INT, mem_dspace, file_dspace,
                H5P_DEFAULT, &mpi.niter[0])) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_mpi_conv, H5T_NATIVE_INT, mem_dspace, file_dspace,
                H5P_DEFAULT, &mpi.convergence[0])) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_mpi_zc, H5T_NATIVE_INT, mem_dspace, file_dspace,
                H5P_DEFAULT, &mpi.zcut_hist[0])) < 0)
    HERR(routineName);
  if ((H5Dwrite(io.in_mpi_dm, H5T_NATIVE_DOUBLE, mem_dspace, file_dspace,
                H5P_DEFAULT, &mpi.dpopsmax[0])) < 0)
    HERR(routineName);
  if ((H5Sclose(file_dspace)) < 0)
    HERR(routineName);
  if ((H5Sclose(mem_dspace)) < 0)
  ...
  if ((H5Sselect_hyperslab(file_dspace, H5S_SELECT_SET, offset, NULL, count,
    ...
    HERR(routineName);
  if ((H5Dwrite(io.in_mpi_dmh, H5T_NATIVE_DOUBLE, mem_dspace, file_dspace,
                H5P_DEFAULT, mpi.dpopsmax_hist[0])) < 0)
    HERR(routineName);
  if ((H5Sclose(file_dspace)) < 0)
    HERR(routineName);
  if ((H5Sclose(mem_dspace)) < 0)
...
void readConvergence(void) {
  ...
  if ((ncid_mpi = H5Gopen(ncid, "mpi", H5P_DEFAULT)) < 0)
    HERR(routineName);
  ...
  if (!strstr(atmosID, atmos.ID)) {
    sprintf(messageStr,
            "Indata file was calculated for different "
            "atmosphere (%s) than current (%s)",
            atmosID, atmos.ID);
    Error(WARNING, routineName, messageStr);
  }
  free(atmosID);
  /* Check that dimension sizes match */
  if ((H5LTget_attribute_int(ncid, "/", "nx", &nx)) < 0)
    HERR(routineName);
  if (nx != mpi.nx) {
    sprintf(messageStr, "Number of x points mismatch: expected %d, found %d.",
            mpi.nx, (int)nx);
    Error(WARNING, routineName, messageStr);
  }
  if ((H5LTget_attribute_int(ncid, "/", "ny", &ny)) < 0)
    HERR(routineName);
  if (ny != mpi.ny) {
    sprintf(messageStr, "Number of y points mismatch: expected %d, found %d.",
            mpi.ny, (int)ny);
    Error(WARNING, routineName, messageStr);
  }
  /* --- Read variable --- */
  if ((H5LTread_dataset_int(ncid_mpi, CONV_NAME, mpi.rh_converged[0])) < 0)
    HERR(routineName);
  /* --- Close inputdata file --- */
  if ((H5Gclose(ncid_mpi)) < 0)
    HERR(routineName);
  if ((H5Fclose(ncid)) < 0)
    HERR(routineName);
  return;
...
void readSavedInput(void) {
  ...
  if (H5LTfind_dataset(ncid_input, "atoms_file_contents")) {
    /* For H5T_STRING datasets, size of string is saved under last argument */
    if ((H5LTget_dataset_info(ncid_input, "atoms_file_contents", NULL, NULL,
                              &str_size)) < 0)
      HERR(routineName);
  } else {
  ...
  if ((H5LTread_dataset_string(ncid_input, "atoms_file_contents",
                               input.atoms_file_contents)) < 0)
    HERR(routineName);
  if ((H5LTget_attribute_int(ncid_input, ".", "natoms", &input.Natoms)) < 0)
    HERR(routineName);
  /* Load atom files */
  ...
  if ((H5LTread_dataset(ncid_input, "atom_groups", H5T_C_S1, atom_names[0])) <
      0)
    HERR(routineName);
  ...
  for (n = 0; n < input.Natoms; n++) {
    ...
    if (H5LTfind_dataset(ncid_tmp, "file_contents")) {
      ...
      if ((H5LTget_dataset_info(ncid_tmp, "file_contents", NULL, NULL,
                                &str_size)) < 0)
        HERR(routineName);
    } else {
    ...
    if ((H5LTread_dataset_string(ncid_tmp, "file_contents",
                                 input.atomic_file_contents[n])) < 0)
      HERR(routineName);
    if ((H5Gclose(ncid_tmp)) < 0)
      HERR(routineName);
  }
  ...
  if ((H5LTget_attribute_double(ncid_input, ".", INPUT_MU, &io.ray_muz)) < 0)
    HERR(routineName);
  if ((H5LTget_attribute_uint(ncid_input, ".", WAVE_SEL,
                              (unsigned int *)&io.ray_nwave_sel)) < 0)
    HERR(routineName);
  ...
  if (H5LTfind_dataset(ncid_input, WAVETABLE)) {
    if ((H5LTget_attribute_uint(ncid_input, ".", NXWAVE,
                                (unsigned int *)&input.Nxwave)) < 0)
      HERR(routineName);
    input.wavetable = (double *)malloc(input.Nxwave * sizeof(double));
    if ((H5LTread_dataset_double(ncid_input, WAVETABLE, input.wavetable)) < 0)
      HERR(routineName);
  }
  /* Data from Kurucz line file, if existing */
  if ((H5LTget_attribute_uint(ncid_input, ".", NKURUCZ,
                              (unsigned int *)&input.Nkurucz_files)) < 0)
    HERR(routineName);
  if (input.Nkurucz_files > 0) {
    if (H5LTfind_dataset(ncid_input, "kurucz_file_contents")) {
      if ((H5LTget_dataset_info(ncid_input, "kurucz_file_contents", NULL, NULL,
                                &str_size)) < 0)
        HERR(routineName);
    } else {
    ...
    if ((H5LTread_dataset_string(ncid_input, "kurucz_file_contents",
                                 input.kurucz_file_contents)) < 0)
      HERR(routineName);
    input.kurucz_line_file_contents =
        (char **)malloc(input.Nkurucz_files * sizeof(char *));
    for (n = 0; n < input.Nkurucz_files; n++) {
      ...
      if ((H5LTread_dataset_string(ncid_tmp, "file_contents",
                                   input.kurucz_line_file_contents[n])) < 0)
        HERR(routineName);
      if ((H5Gclose(ncid_tmp)) < 0)
        HERR(routineName);
    }
