...
void har_mean_deriv(double *wprime, double dsup, double dsdn, double chiup,
...
void Piecewise_1D(int nspect, int mu, bool_t to_obs, double *chi, double *S,
                  double *I, double *Psi) {
  register int k;
...
void Piecewise_Hermite_1D(int nspect, int mu, bool_t to_obs, double *chi,
  ...
  /* upwind path_length */
  dtau_uw = 0.5 * dsup * (chi[k - dk] + chi[k]) +
            1. / 12. * dsup * dsup * (dchi_up - dchi_c);
  ...
  for (k = k_start + dk; k != k_end + dk; k += dk) {
    ...
    if (k != k_end) {
      ...
      dsdn =
          fabs(geometry.height[k + dk] - geometry.height[k]) / geometry.muz[mu];

      /* dchi/ds at downwind point */
      if (abs(k - k_end) > 1) {
        dsdn2 = fabs(geometry.height[k + 2 * dk] - geometry.height[k + dk]) /
                geometry.muz[mu];
      ...
      dtau_dw = 0.5 * dsdn * (chi[k] + chi[k + dk]) +
                1. / 12. * dsdn * dsdn * (dchi_c - dchi_dn);
...
void PieceBezier_1D(int nspect, int mu, bool_t to_obs, double *chi, double *S,
                    double *I, double *Psi) {
  const char routineName[] = "PieceBezier_1D";
  register int k;

  int k_start, k_end, dk, Ndep = geometry.Ndep;
  double dtau_uw, dtau_dw, dS_uw, I_uw, dS_dw, U[3], zmu, Bnu[2], psi_uw, psi_0,
      psi_dw, Sc;
