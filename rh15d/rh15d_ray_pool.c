...
void overlord(void) {
  ...
  if (mpi.total_tasks == 1) {
    ...
    MPI_Sendrecv(&current_task, 1, MPI_LONG, 1, WORKTAG, &result, 1, MPI_INT, 1,
    ...
    /* Loop over getting new work requests until there is no more work to be
     * done */
...
void drone(void) {
  ...
  while (1) {
    ...
    MPI_Recv(&mpi.task, 1, MPI_LONG, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
    ...
    sprintf(messageStr,
            ...
            task - 1, mpi.xnum[mpi.ix], mpi.ynum[mpi.iy]);
    fprintf(mpi.main_logfile, "%s", messageStr);
    Error(MESSAGE, "main", messageStr);
    ...
    if (isnan(mpi.dpopsmax[mpi.task]) || isinf(mpi.dpopsmax[mpi.task]) ||
        ...
        ((mpi.dpopsmax[mpi.task] == 0) && (input.NmaxIter > 0)))
      mpi.stop = TRUE;
