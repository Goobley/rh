...
/* Creates the file for the ray */
{
  ...
  if ((H5LTset_attribute_ushort(ncid, "/", "snapshot_number",
                                (const unsigned short *)&mpi.snap_number, 1)) <
      0)
    HERR(routineName);
  ...
  if ((H5LTset_attribute_int(ncid, "/", "nwave", &spectrum.Nspect, 1)) < 0)
    HERR(routineName);
  if (write_xtra) {
    if ((H5LTset_attribute_int(ncid, "/", WAVE_SEL, &io.ray_nwave_sel, 1)) < 0)
      HERR(routineName);
  }
  ...
  if ((H5LTmake_dataset(ncid, X_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                        geometry.xscale)) < 0)
    HERR(routineName);
  if ((id_x = H5Dopen2(ncid, X_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  dims[0] = mpi.ny;
  if ((H5LTmake_dataset(ncid, Y_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                        geometry.yscale)) < 0)
    HERR(routineName);
  if ((id_y = H5Dopen2(ncid, Y_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  dims[0] = infile.nz;
  tmp = (double *)calloc(infile.nz, sizeof(double));
  if ((H5LTmake_dataset(ncid, ZOUT_NAME, 1, dims, H5T_NATIVE_DOUBLE, tmp)) < 0)
    HERR(routineName);
  ...
  if ((id_wave = H5Dopen2(ncid, WAVE_NAME, H5P_DEFAULT)) < 0)
    HERR(routineName);
  if (write_xtra) {
    ...
    if ((H5LTmake_dataset(ncid, WAVE_SEL_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                          wave_selected)) < 0)
      HERR(routineName);
    if ((id_wave_sel = H5Dopen2(ncid, WAVE_SEL_NAME, H5P_DEFAULT)) < 0)
      HERR(routineName);
  }
  free(wave_selected);
  ...
  if ((H5Pset_fill_time(plist, H5D_FILL_TIME_ALLOC)) < 0)
    HERR(routineName);
  /* Intensity */
  if ((io.ray_int_var = H5Dcreate(ncid, INT_NAME, H5T_NATIVE_FLOAT, file_dspace,
                                  H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
    HERR(routineName);
  /* Attach dimension scales */
  if ((H5DSattach_scale(io.ray_int_var, id_x, 0)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.ray_int_var, id_y, 1)) < 0)
    HERR(routineName);
  if ((H5DSattach_scale(io.ray_int_var, id_wave, 2)) < 0)
    HERR(routineName);

  /* Other Stokes parameters, if available */
  if (atmos.Stokes || input.backgr_pol) {
    if ((io.ray_stokes_q_var =
             H5Dcreate(ncid, STOKES_Q, H5T_NATIVE_FLOAT, file_dspace,
                       H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
      HERR(routineName);
    /* Attach dimension scales */
    if ((H5DSattach_scale(io.ray_stokes_q_var, id_x, 0)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_stokes_q_var, id_y, 1)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_stokes_q_var, id_wave, 2)) < 0)
      HERR(routineName);

    if ((io.ray_stokes_u_var =
             H5Dcreate(ncid, STOKES_U, H5T_NATIVE_FLOAT, file_dspace,
                       H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
      HERR(routineName);
    /* Attach dimension scales */
    if ((H5DSattach_scale(io.ray_stokes_u_var, id_x, 0)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_stokes_u_var, id_y, 1)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_stokes_u_var, id_wave, 2)) < 0)
      HERR(routineName);

    if ((io.ray_stokes_v_var =
             H5Dcreate(ncid, STOKES_V, H5T_NATIVE_FLOAT, file_dspace,
                       H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
      HERR(routineName);
    /* Attach dimension scales */
    if ((H5DSattach_scale(io.ray_stokes_v_var, id_x, 0)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_stokes_v_var, id_y, 1)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_stokes_v_var, id_wave, 2)) < 0)
      HERR(routineName);
  }
  if (input.p15d_wtau) {
    if ((io.ray_tau1_var =
             H5Dcreate(ncid, TAU1_NAME, H5T_NATIVE_FLOAT, file_dspace,
                       H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
      HERR(routineName);
    /* Attach dimension scales */
    if ((H5DSattach_scale(io.ray_tau1_var, id_x, 0)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_tau1_var, id_y, 1)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_tau1_var, id_wave, 2)) < 0)
      HERR(routineName);
  }
  if (write_xtra) {
    dims[0] = mpi.nx;
    ...
    if ((io.ray_chi_var =
             H5Dcreate(ncid, CHI_NAME, H5T_NATIVE_FLOAT, file_dspace,
                       H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
      HERR(routineName);
    /* Attach dimension scales */
    if ((H5DSattach_scale(io.ray_chi_var, id_x, 0)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_chi_var, id_y, 1)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_chi_var, id_z, 2)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_chi_var, id_wave_sel, 3)) < 0)
      HERR(routineName);
    if ((io.ray_S_var = H5Dcreate(ncid, S_NAME, H5T_NATIVE_FLOAT, file_dspace,
                                  H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
      HERR(routineName);
    /* Attach dimension scales */
    if ((H5DSattach_scale(io.ray_S_var, id_x, 0)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_S_var, id_y, 1)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_S_var, id_z, 2)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_S_var, id_wave_sel, 3)) < 0)
      HERR(routineName);
    if ((io.ray_j_var =
             H5Dcreate(ncid, "Jlambda", H5T_NATIVE_FLOAT, file_dspace,
                       H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
      HERR(routineName);
    /* Attach dimension scales */
    if ((H5DSattach_scale(io.ray_j_var, id_x, 0)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_j_var, id_y, 1)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_j_var, id_z, 2)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_j_var, id_wave_sel, 3)) < 0)
      HERR(routineName);
    if ((io.ray_sca_c_var =
             H5Dcreate(ncid, SCA_C_NAME, H5T_NATIVE_FLOAT, file_dspace,
                       H5P_DEFAULT, plist, H5P_DEFAULT)) < 0)
      HERR(routineName);
    /* Attach dimension scales */
    if ((H5DSattach_scale(io.ray_sca_c_var, id_x, 0)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_sca_c_var, id_y, 1)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_sca_c_var, id_z, 2)) < 0)
      HERR(routineName);
    if ((H5DSattach_scale(io.ray_sca_c_var, id_wave_sel, 3)) < 0)
      HERR(routineName);
  }
  ...
  if ((H5LTset_attribute_string(ncid, "/", "creation_time",
                                (const char *)&timestr)) < 0)
    HERR(routineName);

  /*  units and fill value  */
  if ((H5LTset_attribute_string(ncid, X_NAME, "units", UNIT_LENGTH)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_string(ncid, Y_NAME, "units", UNIT_LENGTH)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_string(ncid, ZOUT_NAME, "units", UNIT_LENGTH)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_string(ncid, WAVE_NAME, "units", UNIT_WAVE)) < 0)
    HERR(routineName);
  if (write_xtra)
    if ((H5LTset_attribute_string(ncid, WAVE_SEL_NAME, "units", UNIT_WAVE)) < 0)
      HERR(routineName);
  if ((H5LTset_attribute_string(ncid, INT_NAME, "units", UNIT_INTENSITY)) < 0)
    HERR(routineName);
  if ((H5LTset_attribute_float(ncid, INT_NAME, "_FillValue", &FILLVALUE, 1)) <
      0)
    HERR(routineName);

  if (atmos.Stokes || input.backgr_pol) {
    if ((H5LTset_attribute_string(ncid, STOKES_Q, "units", UNIT_INTENSITY)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_float(ncid, STOKES_Q, "_FillValue", &FILLVALUE, 1)) <
        0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid, STOKES_U, "units", UNIT_INTENSITY)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_float(ncid, STOKES_U, "_FillValue", &FILLVALUE, 1)) <
        0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid, STOKES_V, "units", UNIT_INTENSITY)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_float(ncid, STOKES_V, "_FillValue", &FILLVALUE, 1)) <
        0)
      HERR(routineName);
  }

  if (input.p15d_wtau) {
    if ((H5LTset_attribute_string(ncid, TAU1_NAME, "units", UNIT_LENGTH)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid, TAU1_NAME, DESC_NAME,
                                  "Height of optical depth unity")) < 0)
      HERR(routineName);
  }

  if (write_xtra) {
    if ((H5LTset_attribute_string(ncid, S_NAME, "units", UNIT_INTENSITY)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid, S_NAME, DESC_NAME,
                                  "Total source function (line + continuum)")) <
        0)
      HERR(routineName);
    if ((H5LTset_attribute_float(ncid, S_NAME, "_FillValue", &FILLVALUE, 1)) <
        0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid, CHI_NAME, "units", UNIT_PER_LENGTH)) <
        0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid, CHI_NAME, DESC_NAME,
                                  "Total absorption (line + continuum)")) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_float(ncid, CHI_NAME, "_FillValue", &FILLVALUE, 1)) <
        0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid, "/Jlambda", "units", UNIT_INTENSITY)) <
        0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid, "/Jlambda", DESC_NAME,
                                  "Mean radiation field")) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_float(ncid, "/Jlambda", "_FillValue", &FILLVALUE,
                                 1)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid, SCA_C_NAME, DESC_NAME,
                                  "Scattering term multiplied by Jlambda")) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_float(ncid, SCA_C_NAME, "_FillValue", &FILLVALUE,
                                 1)) < 0)
      HERR(routineName);
  }
  if ((H5Pclose(plist)) < 0)
  ...
  if ((H5Dclose(id_z)) < 0)
    HERR(routineName);
  if ((H5Dclose(id_wave)) < 0)
    HERR(routineName);
  if (write_xtra)
    if ((H5Dclose(id_wave_sel)) < 0)
      HERR(routineName);
...
/* Opens an existing ray file */
{
  ...
  if ((H5Pset_fapl_mpio(plist, mpi.comm, mpi.info)) < 0)
    HERR(routineName);
  if ((ncid = H5Fopen(RAY_FILE, H5F_ACC_RDWR, plist)) < 0)
    HERR(routineName);
  if ((H5Pclose(plist)) < 0)
    HERR(routineName);
...
void close_hdf5_ray(void) {
  ...
  write_xtra = (io.ray_nwave_sel > 0);
  /* Close all datasets */
  if ((H5Dclose(io.ray_int_var)) < 0)
    HERR(routineName);
  if (atmos.Stokes || input.backgr_pol) {
    if ((H5Dclose(io.ray_stokes_q_var)) < 0)
      HERR(routineName);
    if ((H5Dclose(io.ray_stokes_u_var)) < 0)
      HERR(routineName);
    if ((H5Dclose(io.ray_stokes_v_var)) < 0)
      HERR(routineName);
  }
  if (input.p15d_wtau) {
    if ((H5Dclose(io.ray_tau1_var)) < 0)
      HERR(routineName);
  }
  if (write_xtra) {
    if ((H5Dclose(io.ray_chi_var)) < 0)
      HERR(routineName);
    if ((H5Dclose(io.ray_S_var)) < 0)
      HERR(routineName);
    if ((H5Dclose(io.ray_j_var)) < 0)
      HERR(routineName);
    if ((H5Dclose(io.ray_sca_c_var)) < 0)
      HERR(routineName);
    if (io.ray_wave_idx != NULL)
      free(io.ray_wave_idx);
  }
  if ((H5Fclose(io.ray_ncid)) < 0)
    HERR(routineName);
  return;
}
...
void writeRay(void) {
  ...
  if ((file_dataspace = H5Dget_space(io.ray_int_var)) < 0)
    HERR(routineName);
  if ((H5Sselect_hyperslab(file_dataspace, H5S_SELECT_SET, offset, NULL, count,
                           NULL)) < 0)
    HERR(routineName);

  /* Write intensity */
  if ((H5Dwrite(io.ray_int_var, H5T_NATIVE_DOUBLE, mem_dataspace,
                file_dataspace, H5P_DEFAULT, spectrum.I[0])) < 0)
    HERR(routineName);
  ...
  if (input.p15d_wtau) {
    ...
    if ((H5Dwrite(io.ray_tau1_var, H5T_NATIVE_FLOAT, mem_dataspace,
                  file_dataspace, H5P_DEFAULT, tau_one)) < 0)
      HERR(routineName);
  ...
  if (write_xtra) {
    ...
    count[2] = infile.nz;
    offset[3] = 0;
    count[3] = io.ray_nwave_sel;
    if ((file_dataspace = H5Dget_space(io.ray_S_var)) < 0)
      HERR(routineName);
    if ((H5Sselect_hyperslab(file_dataspace, H5S_SELECT_SET, offset, NULL,
                             count, NULL)) < 0)
      HERR(routineName);
    if ((H5Dwrite(io.ray_chi_var, H5T_NATIVE_FLOAT, mem_dataspace,
                  file_dataspace, H5P_DEFAULT, chi[0])) < 0)
      HERR(routineName);
    if ((H5Dwrite(io.ray_S_var, H5T_NATIVE_FLOAT, mem_dataspace, file_dataspace,
                  H5P_DEFAULT, S[0])) < 0)
      HERR(routineName);
    if ((H5Dwrite(io.ray_j_var, H5T_NATIVE_FLOAT, mem_dataspace, file_dataspace,
                  H5P_DEFAULT, Jnu[0])) < 0)
      HERR(routineName);
    if ((H5Dwrite(io.ray_sca_c_var, H5T_NATIVE_FLOAT, mem_dataspace,
                  file_dataspace, H5P_DEFAULT, sca[0])) < 0)
      HERR(routineName);
    freeMatrix((void **)chi);
    freeMatrix((void **)S);
    freeMatrix((void **)Jnu);
...
void calculate_ray(void) {
  ...
  for (nact = 0; nact < atmos.Nactiveatom; nact++) {
    ...
    */
    ...
    for (i = 0; i < atom->Nline; i++) {
      ...
      if (atmos.moving && line->polarizable &&
          (input.StokesMode > FIELD_FREE)) {
        if (line->phi_Q != NULL) {
  ...
  if (input.PRD_angle_dep == PRD_ANGLE_APPROX && atmos.NPRDactive > 0) {
    ...
    if (spectrum.v_los != NULL)
      freeMatrix((void **)spectrum.v_los);
