...
void init_hdf5_atmos(Atmosphere *atmos, Geometry *geometry,
  ...
  if ((plist_id = H5Pcreate(H5P_FILE_ACCESS)) < 0)
    HERR(routineName);
  if ((H5Pset_fapl_mpio(plist_id, mpi.comm, mpi.info)) < 0)
    HERR(routineName);
  if ((ncid = H5Fopen(input.atmos_input, H5F_ACC_RDONLY, plist_id)) < 0)
    HERR(routineName);
  infile->ncid = ncid;
  if ((H5Pclose(plist_id)) < 0)
    HERR(routineName); /* plist no longer needed */
  ...
  atmos->Stokes = FALSE;
  if ((has_B) && (strcmp(input.Stokes_input, "none")))
    atmos->Stokes = TRUE;

  /* Get dimensions from hydrogen_populations (if present) */
  if (H5LTfind_dataset(ncid, "hydrogen_populations")) {
    if ((H5LTget_dataset_info(ncid, "hydrogen_populations", dims, NULL, NULL)) <
        0)
      HERR(routineName);
    nn = dims[1];
    infile->nx = dims[2];
  ...
  if (atmos->NHydr < 2 && !atmos->H_LTE) {
    sprintf(messageStr,
            "NHydr has to be at least 2, not %d to run with"
  ...
  if (input.solve_ne == NONE) {
    if (H5LTfind_dataset(ncid, NE_NAME)) {
      ...
      sprintf(messageStr,
              "Electron density not found in atmosphere file,"
  ...
  if ((H5LTget_dataset_ndims(ncid, "z", &mpi.ndims_z)) < 0)
    HERR(routineName);
  ...
  if ((H5LTread_dataset_double(ncid, "y", infile->y)) < 0)
    HERR(routineName);
  infile->x = (double *)malloc(infile->nx * sizeof(double));
  if ((H5LTread_dataset_double(ncid, "x", infile->x)) < 0)
    HERR(routineName);
...
void readAtmos_hdf5(int xi, int yi, Atmosphere *atmos, Geometry *geometry,
  ...
  start[3] = 0;
  count[3] = infile->nz;
  if ((dataspace_id = H5Dget_space(infile->T_varid)) < 0)
    HERR(routineName);
  if ((H5Sselect_hyperslab(dataspace_id, H5S_SELECT_SET, start, NULL, count,
                           NULL)) < 0)
    HERR(routineName);
  ...
  ierror = H5Sselect_hyperslab(dataspace_id, H5S_SELECT_SET, start, NULL, count,
                               NULL);
  ...
  /* read nH, all at once */
  start_nh[0] = input.p15d_nt;
  count_nh[0] = 1;
  start_nh[1] = 0;
  count_nh[1] = atmos->NHydr;
  start_nh[2] = (size_t)xi;
  count_nh[2] = 1;
  start_nh[3] = (size_t)yi;
  count_nh[3] = 1;
  start_nh[4] = mpi.zcut;
  count_nh[4] = atmos->Nspace;
  dataspace_id = H5Dget_space(infile->nh_varid);
  ierror = H5Sselect_hyperslab(dataspace_id, H5S_SELECT_SET, start_nh, NULL,
                               count_nh, NULL);
  dims_memory[0] = atmos->NHydr;
  dims_memory[1] = atmos->Nspace;
  memspace_id = H5Screate_simple(2, dims_memory, NULL);
  ierror = H5Dread(infile->nh_varid, H5T_NATIVE_DOUBLE, memspace_id,
                   dataspace_id, H5P_DEFAULT, atmos->nH[0]);
  if ((H5Sclose(dataspace_id)) < 0)
    HERR(routineName);
  if ((H5Sclose(memspace_id)) < 0)
    HERR(routineName);
...
void close_hdf5_atmos(Atmosphere *atmos, Geometry *geometry,
  ...
  if (infile->vturb_varid != -1)
    ierror = H5Dclose(infile->vturb_varid);
