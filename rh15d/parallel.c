...
void initParallel(int *argc, char **argv[], bool_t run_ray) {
  ...
  if ((mpi.logfile = fopen(logfile, "w")) == NULL) {
    sprintf(messageStr, "Process %4d: Unable to open log file %s", mpi.rank,
            logfile);
...
void UpdateAtmosDep(void) {
  ...
  for (nact = 0; nact < atmos.Natom; nact++) {
    ...
    atom->ntotal =
        (double *)realloc(atom->ntotal, atmos.Nspace * sizeof(double));
    atom->vbroad =
        (double *)realloc(atom->vbroad, atmos.Nspace * sizeof(double));
  ...
  for (nact = 0; nact < atmos.Nactiveatom; nact++) {
    ...
    for (kr = 0; kr < atom->Ncont; kr++) {
      atom->continuum[kr].Rij = (double *)realloc(
          atom->continuum[kr].Rij, atmos.Nspace * sizeof(double));
      atom->continuum[kr].Rji = (double *)realloc(
          atom->continuum[kr].Rji, atmos.Nspace * sizeof(double));
      for (k = 0; k < atmos.Nspace; k++) {
    ...
    for (kr = 0; kr < atom->Nline; kr++) {
      ...
      if (line->PRD) {
        ...
        /* Idea: instead of doing this, why not free and just set line->rho_prd
           = NULL, (and also line->Qelast?), because profile.c will act on that
           and reallocate */
        ...
        if (line->Qelast != NULL) {
          line->Qelast =
              (double *)realloc(line->Qelast, atmos.Nspace * sizeof(double));
        } else {
  ...
  for (nact = 0; nact < atmos.Nmolecule; nact++) {
    ...
    /* Reallocate some stuff, because of varying Nspace */
    molecule->vbroad =
        (double *)realloc(molecule->vbroad, atmos.Nspace * sizeof(double));
    molecule->pf =
        (double *)realloc(molecule->pf, atmos.Nspace * sizeof(double));
...
void copyBufVars(bool_t writej) {
  ...
  for (nact = 0; nact < atmos.Nactiveatom; nact++) {
    ...
    memcpy((void *)&iobuf.nstar[nact][ind * atom->Nlevel],
           (void *)atom->nstar[0],
           atom->Nlevel * atmos.Nspace * sizeof(double));
  ...
  for (nact = 0; nact < atmos.Nactivemol; nact++) {
    ...
    /* nv, nvstar */
    memcpy((void *)&iobuf.nv[nact][ind * molecule->Nv], (void *)molecule->nv[0],
...
void writeOutput(bool_t writej) {
  ...
  */
  ...
  if (mpi.Ntasks == 0) {
    sprintf(messageStr,
            "Process %4d: *** NO WORK (more processes than "
            "tasks!)\n",
            mpi.rank);
