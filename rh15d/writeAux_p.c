...
void init_aux_new(void) {
  ...
  if ((H5Pset_fapl_mpio(plist, mpi.comm, mpi.info)) < 0)
    HERR(routineName);
  if ((ncid = H5Fcreate(AUX_FILE, H5F_ACC_TRUNC, H5P_DEFAULT, plist)) < 0)
    HERR(routineName);
  if ((H5Pclose(plist)) < 0)
    HERR(routineName);
  ...
  for (i = 0; i < atmos.Nactiveatom; i++) {
    ...
    /* --- dimensions as attributes --- */
    if ((H5LTset_attribute_int(ncid_atom, ".", "nlevel", &atom->Nlevel, 1)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_int(ncid_atom, ".", "nline", &atom->Nline, 1)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_int(ncid_atom, ".", "ncontinuum", &atom->Ncont, 1)) <
        0)
      HERR(routineName);
    /* --- dimension datasets --- */
    dims[0] = mpi.nx;
    if ((H5LTmake_dataset(ncid_atom, X_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                          geometry.xscale)) < 0)
      HERR(routineName);
    if ((id_x = H5Dopen2(ncid_atom, X_NAME, H5P_DEFAULT)) < 0)
      HERR(routineName);
    dims[0] = mpi.ny;
    if ((H5LTmake_dataset(ncid_atom, Y_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                          geometry.yscale)) < 0)
      HERR(routineName);
    if ((id_y = H5Dopen2(ncid_atom, Y_NAME, H5P_DEFAULT)) < 0)
      HERR(routineName);
    dims[0] = infile.nz;
    ...
    if ((H5LTmake_dataset(ncid_atom, ZOUT_NAME, 1, dims, H5T_NATIVE_DOUBLE,
      ...
      HERR(routineName);
    free(tmp_double);
    if ((id_z = H5Dopen2(ncid_atom, ZOUT_NAME, H5P_DEFAULT)) < 0)
      HERR(routineName);
    dims[0] = atom->Nlevel;
    ...
    if (input.p15d_wpop) {
      if ((file_dspace = H5Screate_simple(4, dims, NULL)) < 0)
        HERR(routineName);
      if ((id_n = H5Dopen2(ncid_atom, LEVEL_NAME, H5P_DEFAULT)) < 0)
        HERR(routineName);
      if (atom->n != NULL) {
        ...
        if ((H5DSattach_scale(id_tmp, id_n, 0)) < 0)
          HERR(routineName);
        if ((H5DSattach_scale(id_tmp, id_x, 1)) < 0)
          HERR(routineName);
        if ((H5DSattach_scale(id_tmp, id_y, 2)) < 0)
          HERR(routineName);
        if ((H5DSattach_scale(id_tmp, id_z, 3)) < 0)
          HERR(routineName);
    ...
    if (input.p15d_wrates) {
      ...
      if ((file_dspace = H5Screate_simple(4, dims, NULL)) < 0)
        HERR(routineName);
      if ((id_n = H5Dopen2(ncid_atom, LINE_NAME, H5P_DEFAULT)) < 0)
        HERR(routineName);
      ...
      if (atom->Ncont > 0) {
        ...
        if ((file_dspace = H5Screate_simple(4, dims, NULL)) < 0)
          HERR(routineName);
        if ((id_n = H5Dopen2(ncid_atom, CONT_NAME, H5P_DEFAULT)) < 0)
          HERR(routineName);
  ...
  for (i = 0; i < atmos.Nactivemol; i++) {
    ...
    /* --- dimensions as attributes --- */
    if ((H5LTset_attribute_int(ncid_mol, ".", "nlevel_vibr", &molecule->Nv,
                               1)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_int(ncid_mol, ".", "nline_molecule", &molecule->Nrt,
                               1)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_int(ncid_mol, ".", "nJ", &molecule->NJ, 1)) < 0)
      HERR(routineName);
    /* --- dimension datasets --- */
    dims[0] = mpi.nx;
    if ((H5LTmake_dataset(ncid_mol, X_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                          geometry.xscale)) < 0)
      HERR(routineName);
    if ((id_x = H5Dopen2(ncid_mol, X_NAME, H5P_DEFAULT)) < 0)
      HERR(routineName);
    dims[0] = mpi.ny;
    if ((H5LTmake_dataset(ncid_mol, Y_NAME, 1, dims, H5T_NATIVE_DOUBLE,
                          geometry.yscale)) < 0)
      HERR(routineName);
    if ((id_y = H5Dopen2(ncid_mol, Y_NAME, H5P_DEFAULT)) < 0)
      HERR(routineName);
    dims[0] = infile.nz;
    tmp_double = (double *)calloc(infile.nz, sizeof(double));
    if ((H5LTmake_dataset(ncid_mol, ZOUT_NAME, 1, dims, H5T_NATIVE_DOUBLE,
      ...
      HERR(routineName);
    free(tmp_double);
    if ((id_z = H5Dopen2(ncid_mol, ZOUT_NAME, H5P_DEFAULT)) < 0)
      HERR(routineName);
    dims[0] = molecule->Nv;
    ...
    if ((H5LTmake_dataset(ncid_mol, NJ_NAME, 1, dims, H5T_NATIVE_UINT, tmp)) <
        0)
      HERR(routineName);
    free(tmp);
    /* For compatibility with netCDF readers, only use dataset as dimension */
    if ((H5LTset_attribute_string(ncid_mol, ZOUT_NAME, "NAME", NETCDF_COMPAT)) <
        0)
      HERR(routineName);
    ...
    if ((H5LTset_attribute_string(ncid_mol, VLINE_NAME, "NAME",
                                  NETCDF_COMPAT)) < 0)
      HERR(routineName);
    if ((H5LTset_attribute_string(ncid_mol, NJ_NAME, "NAME", NETCDF_COMPAT)) <
        0)
      HERR(routineName);
    /* --- variables --- */
    dims[0] = molecule->Nv;
    ...
    if (input.p15d_wpop) {
      if ((file_dspace = H5Screate_simple(4, dims, NULL)) < 0)
        HERR(routineName);
      if ((id_n = H5Dopen2(ncid_mol, VLEVEL_NAME, H5P_DEFAULT)) < 0)
        HERR(routineName);
      if (molecule->nv != NULL) {
  ...
  if ((H5Pclose(plist)) < 0)
    HERR(routineName); /* Free hdf5 resources */
...
void init_aux_existing(void) {
  ...
  for (i = 0; i < atmos.Nactiveatom; i++) {
    ...
    sprintf(group_name, (atom->ID[1] == ' ') ? "atom_%.1s" : "atom_%.2s",
            atom->ID);
    if ((io.aux_atom_ncid[i] = H5Gopen(io.aux_ncid, group_name, H5P_DEFAULT)) <
        0)
      HERR(routineName);
    ...
    if (nlevel != atom->Nlevel) {
      sprintf(messageStr, "Number of levels mismatch: expected %d, found %d.",
    ...
    if ((H5LTget_attribute_int(ncid_atom, ".", "nline", &nline)) < 0)
      HERR(routineName);
    if (nline != atom->Nline) {
      sprintf(messageStr, "Number of lines mismatch: expected %d, found %d.",
    ...
    if ((H5LTget_attribute_int(ncid_atom, ".", "ncontinuum", &ncont)) < 0)
      HERR(routineName);
    if (ncont != atom->Ncont) {
      sprintf(messageStr, "Number of continua mismatch: expected %d, found %d.",
    ...
    if (input.p15d_wpop) {
      if ((io.aux_atom_pop[i] = H5Dopen(ncid_atom, POP_NAME, H5P_DEFAULT)) < 0)
        HERR(routineName);
      if ((io.aux_atom_poplte[i] =
               H5Dopen(ncid_atom, POPLTE_NAME, H5P_DEFAULT)) < 0)
        HERR(routineName);
    }
    if (input.p15d_wrates) {
      if ((io.aux_atom_RijL[i] = H5Dopen(ncid_atom, RIJ_L_NAME, H5P_DEFAULT)) <
          0)
        HERR(routineName);
      if ((io.aux_atom_RjiL[i] = H5Dopen(ncid_atom, RJI_L_NAME, H5P_DEFAULT)) <
          0)
        HERR(routineName);
      if (atom->Ncont > 0) {
        if ((io.aux_atom_RijC[i] =
                 H5Dopen(ncid_atom, RIJ_C_NAME, H5P_DEFAULT)) < 0)
          HERR(routineName);
        if ((io.aux_atom_RjiC[i] =
                 H5Dopen(ncid_atom, RJI_C_NAME, H5P_DEFAULT)) < 0)
          HERR(routineName);
      }
    }
  } /* end active ATOMS loop */
  ...
  for (i = 0; i < atmos.Nactivemol; i++) {
    ...
    sprintf(group_name, "molecule_%s", molecule->ID);
    if ((io.aux_mol_ncid[i] = H5Gopen(io.aux_ncid, group_name, H5P_DEFAULT)) <
        0)
      HERR(routineName);
    ...
    if (ncont != molecule->NJ) {
      sprintf(messageStr, "Number of J mismatch: expected %d, found %d.",
    ...
    /* --- Open datasets collectively ---*/
    if (input.p15d_wpop) {
      if ((io.aux_mol_pop[i] = H5Dopen(ncid_mol, POP_NAME, H5P_DEFAULT)) < 0)
        HERR(routineName);
      if ((io.aux_mol_poplte[i] = H5Dopen(ncid_mol, POPLTE_NAME, H5P_DEFAULT)) <
          0)
        HERR(routineName);
    }
  } /* end active MOLECULES loop */
...
/* Closes the aux file */
{
  ...
  for (i = 0; i < atmos.Nactiveatom; i++) {
    atom = atmos.activeatoms[i];
    if (input.p15d_wpop) {
      if ((H5Dclose(io.aux_atom_pop[i])) < 0)
        HERR(routineName);
      if ((H5Dclose(io.aux_atom_poplte[i])) < 0)
        HERR(routineName);
    }
    if (input.p15d_wrates) {
      if ((H5Dclose(io.aux_atom_RijL[i])) < 0)
        HERR(routineName);
      if ((H5Dclose(io.aux_atom_RjiL[i])) < 0)
        HERR(routineName);
      if (atom->Ncont > 0) {
        if ((H5Dclose(io.aux_atom_RijC[i])) < 0)
          HERR(routineName);
        if ((H5Dclose(io.aux_atom_RjiC[i])) < 0)
          HERR(routineName);
      }
    }
    if ((H5Gclose(io.aux_atom_ncid[i])) < 0)
      HERR(routineName);
  }
  for (i = 0; i < atmos.Nactivemol; i++) {
    if (input.p15d_wpop) {
      if ((H5Dclose(io.aux_mol_pop[i])) < 0)
        HERR(routineName);
      if ((H5Dclose(io.aux_mol_poplte[i])) < 0)
        HERR(routineName);
    }
    if ((H5Gclose(io.aux_mol_ncid[i])) < 0)
      HERR(routineName);
  }

  /* Close file */
  if ((H5Fclose(io.aux_ncid)) < 0)
    HERR(routineName);
  return;
}
    ...
    for (task = 0; task < mpi.Ntasks; task++) {
      ...
      if (input.p15d_wrates) {
        ...
        if ((H5Dwrite(io.aux_atom_RijL[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                      file_dspace, H5P_DEFAULT,
                      &iobuf.RijL[nact][ind * atom->Nline])) < 0)
          HERR(routineName);
        if ((H5Dwrite(io.aux_atom_RjiL[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                      file_dspace, H5P_DEFAULT,
                      &iobuf.RjiL[nact][ind * atom->Nline])) < 0)
          HERR(routineName);
        /* release dataspace resources */
        if ((H5Sclose(mem_dspace)) < 0)
          HERR(routineName);
        ...
        if (atom->Ncont > 0) {
          ...
          if ((H5Dwrite(io.aux_atom_RijC[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                        file_dspace, H5P_DEFAULT,
                        &iobuf.RijC[nact][ind * atom->Ncont])) < 0)
            HERR(routineName);
          if ((H5Dwrite(io.aux_atom_RjiC[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                        file_dspace, H5P_DEFAULT,
                        &iobuf.RjiC[nact][ind * atom->Ncont])) < 0)
            HERR(routineName);
          /* release dataspace resources */
          if ((H5Sclose(mem_dspace)) < 0)
            HERR(routineName);
    ...
    for (task = 0; task < mpi.Ntasks; task++) {
      ...
      if (input.p15d_wpop) {
        ...
        if ((H5Dwrite(io.aux_mol_pop[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                      file_dspace, H5P_DEFAULT,
                      &iobuf.nv[nact][ind * molecule->Nv])) < 0)
          HERR(routineName);
        if ((H5Dwrite(io.aux_mol_poplte[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                      file_dspace, H5P_DEFAULT,
                      &iobuf.nvstar[nact][ind * molecule->Nv])) < 0)
          HERR(routineName);
        /* release dataspace resources */
        if ((H5Sclose(mem_dspace)) < 0)
          HERR(routineName);
  ...
  for (nact = 0; nact < atmos.Nactiveatom; nact++) {
    ...
    if (input.p15d_wpop) {
      ...
      if ((H5Sselect_hyperslab(file_dspace, H5S_SELECT_SET, offset, NULL, count,
        ...
        HERR(routineName);
      if ((H5Dwrite(io.aux_atom_pop[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                    file_dspace, H5P_DEFAULT, atom->n[0])) < 0)
        HERR(routineName);
      if ((H5Dwrite(io.aux_atom_poplte[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                    file_dspace, H5P_DEFAULT, atom->nstar[0])) < 0)
        HERR(routineName);
      if ((H5Sclose(mem_dspace)) < 0)
        HERR(routineName);
      if ((H5Sclose(file_dspace)) < 0)
    ...
    if (input.p15d_wrates) {
      ...
      for (kr = 0; kr < atom->Nline; kr++) {
        ...
        if ((H5Sselect_hyperslab(file_dspace, H5S_SELECT_SET, offset, NULL,
          ...
          HERR(routineName);
        if ((H5Dwrite(io.aux_atom_RijL[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                      file_dspace, H5P_DEFAULT, line->Rij)) < 0)
          HERR(routineName);
        if ((H5Dwrite(io.aux_atom_RjiL[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                      file_dspace, H5P_DEFAULT, line->Rji)) < 0)
          HERR(routineName);
      }
      for (kr = 0; kr < atom->Ncont; kr++) {
        ...
        if ((H5Sselect_hyperslab(file_dspace, H5S_SELECT_SET, offset, NULL,
          ...
          HERR(routineName);
        if ((H5Dwrite(io.aux_atom_RijC[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                      file_dspace, H5P_DEFAULT, continuum->Rij)) < 0)
          HERR(routineName);
        if ((H5Dwrite(io.aux_atom_RjiC[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                      file_dspace, H5P_DEFAULT, continuum->Rji)) < 0)
          HERR(routineName);
      }
      if ((H5Sclose(mem_dspace)) < 0)
        HERR(routineName);
  ...
  for (nact = 0; nact < atmos.Nactivemol; nact++) {
    ...
    if (input.p15d_wpop) {
      ...
      if ((H5Sselect_hyperslab(file_dspace, H5S_SELECT_SET, offset, NULL, count,
        ...
        HERR(routineName);
      if ((H5Dwrite(io.aux_mol_pop[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                    file_dspace, H5P_DEFAULT, molecule->nv[0])) < 0)
        HERR(routineName);
      if ((H5Dwrite(io.aux_mol_poplte[nact], H5T_NATIVE_DOUBLE, mem_dspace,
                    file_dspace, H5P_DEFAULT, molecule->nvstar[0])) < 0)
        HERR(routineName);
      if ((H5Sclose(mem_dspace)) < 0)
        HERR(routineName);
      if ((H5Sclose(file_dspace)) < 0)
...
void readPopulations(Atom *atom) {
  ...
  count[0] = atom->Nlevel;
  count[3] = atmos.Nspace;
  if ((file_dspace = H5Dget_space(pop_var)) < 0)
    HERR(routineName);
  if ((H5Sselect_hyperslab(file_dspace, H5S_SELECT_SET, offset, NULL, count,
                           NULL)) < 0)
    HERR(routineName);
  if ((H5Dread(pop_var, H5T_NATIVE_DOUBLE, mem_dspace, file_dspace, H5P_DEFAULT,
               atom->n[0])) < 0)
    HERR(routineName);
  if ((H5Sclose(mem_dspace)) < 0)
    HERR(routineName);
