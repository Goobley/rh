  ...
  if (file_hdf5 > 0) { /* File is HDF5 */
    ...
    if ((fp_atmos = fopen(input.atmos_input, "r")) == NULL) {
      ...
      case 'H':
        ...
        break;
      default:
        sprintf(messageStr, "Failed to read %s as MULTI atmosphere failed.\n",
                input.atmos_input);
        Error(ERROR_LEVEL_2, routineName, messageStr);
      }
    }
...
void realloc_ndep(Atmosphere *atmos, Geometry *geometry) {
  ...
  atmos->nHtot =
      (double *)realloc(atmos->nHtot, atmos->Nspace * sizeof(double));
  geometry->vel =
      (double *)realloc(geometry->vel, atmos->Nspace * sizeof(double));
  geometry->height =
      (double *)realloc(geometry->height, atmos->Nspace * sizeof(double));
  if (atmos->nHmin != NULL)
    atmos->nHmin =
        (double *)realloc(atmos->nHmin, atmos->Nspace * sizeof(double));
  /* default zero */
  free(atmos->vturb);
  ...
  if (atmos->Stokes) {
    ...
    atmos->gamma_B =
        (double *)realloc(atmos->gamma_B, atmos->Nspace * sizeof(double));
    atmos->chi_B =
        (double *)realloc(atmos->chi_B, atmos->Nspace * sizeof(double));
  }
}
...
void depth_refine(Atmosphere *atmos, Geometry *geometry, double Tmax) {
  ...
  */
  ...
  if (Tmax < 0)
    Tmax = 1.e20; /* No zcut if Tmax is negative */
  ...
  for (k = 1; k < atmos->Nspace; k++) {
    if ((atmos->T[k] > Tmax) && (k0 == k - 1))
      k0 = k;
    tau[k] =
        tau[k - 1] + 0.5 * (chi[k] + chi[k - 1]) *
