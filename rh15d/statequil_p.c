  ...
  for (k = 0; k < atmos.Nspace; k++) {
    if (molecule->n[k] > 0.0) {
      ...
      for (vi = 0; vi < Nlevel; vi++) {
        ...
        for (vj = 0; vj < Nlevel; vj++)
          GamDiag += Gamma_k[vj][vi];
      ...
      for (vi = 0; vi < Nlevel; vi++)
        molecule->nv[vi][k] = n_k[vi];
    } else
      for (vi = 0; vi < Nlevel; vi++)
        molecule->nv[vi][k] = 0.0;
  }
...
double updatePopulations(int niter) {
  ...
  for (nact = 0; nact < atmos.Nactiveatom; nact++) {
    ...
    if (mpi.stop)
      return 1.; /* Get out if there is a singular matrix */
