...
void Background_p(bool_t write_analyze_output, bool_t equilibria_only) {
  ...
  if (input.backgr_in_mem) {
    ...
    if (input.StokesMode != NO_STOKES)
      Error(ERROR_LEVEL_2, routineName,
            "Polarized transfer and in-memory backgr. op. is not implemented.");

    if (atmos.chi_b != NULL)
      freeMatrix((void **)atmos.chi_b);
    atmos.chi_b =
        matrix_double(spectrum.Nspect * 2 * atmos.Nrays, atmos.Nspace);

    if (atmos.eta_b != NULL)
      freeMatrix((void **)atmos.eta_b);
    atmos.eta_b =
        matrix_double(spectrum.Nspect * 2 * atmos.Nrays, atmos.Nspace);

    if (atmos.sca_b != NULL)
      freeMatrix((void **)atmos.sca_b);
    atmos.sca_b =
        matrix_double(spectrum.Nspect * 2 * atmos.Nrays, atmos.Nspace);
    ...
    if ((atmos.fd_background = open(file_background, O_RDWR | O_CREAT | O_TRUNC,
                                    PERMISSIONS)) == -1) {
      sprintf(messageStr, "Unable to open output file %s", file_background);
      Error(ERROR_LEVEL_2, routineName, messageStr);
    }
  ...
  for (nspect = 0; nspect < spectrum.Nspect; nspect++) {
    ...
    if (atmos.moving || atmos.Stokes) {
      ...
      if (input.backgr_in_mem) {
        ...
        mpi.backgrrecno +=
            writeBackground(nspect, 0, 0, chi_c, eta_c, sca_c, NULL);
      }
    }
  }
  ...
  if (geometry.atmos_format == MULTI)
