...
void PRDAngleScatter(AtomicLine *PRDline, enum Interpolation representation) {
  ...
  double *v_emit, v0, vN, *v_abs = NULL, *vp = NULL, *wv = NULL, *rii = NULL,
                          *adamp, *Jbar, cDop, *RIInorm, *I = NULL, **Imup, *Ik,
                          *Pj, *gamma, **v_los, *phi_emit, wmup, *sv;
  Atom *atom;
  AtomicLine *line;
  AtomicContinuum *continuum;
  ...
  for (k = 0; k < atmos.Nspace; k++) {
    gamma[k] =
        atom->n[PRDline->i][k] / atom->n[PRDline->j][k] * PRDline->Bij / Pj[k];
  }
          ...
          for (to_obs_p = 0; to_obs_p <= 1; to_obs_p++) {
            ...
            for (k = 0; k < atmos.Nspace; k++) {
              for (lap = 0; lap < PRDline->Nlambda; lap++) {
                ...
                if (to_obs_p)
                  v_abs[lap] = (PRDline->lambda[lap] - PRDline->lambda0) *
                                   CLIGHT /
                                   (PRDline->lambda0 * atom->vbroad[k]) +
                               v_los[mup][k];
                else
              ...
              if (initialize) {
                ...
                if ((Nwrite = fwrite(rii, sizeof(double), Np,
                                     PRDline->fp_GII)) != Np) {
                  sprintf(messageStr,
                          "Unable to write proper number of redistribution "
                          "weights\n"
                          " Wrote %d instead of %d.\n Line %d -> %d, la = %d, "
                          "k = %d",
                ...
                if ((Nread = fread(rii, sizeof(double), Np, PRDline->fp_GII)) !=
                    Np) {
                  sprintf(
                      messageStr,
                      "Unable to read proper number of redistribution weights\n"
