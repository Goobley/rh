...
void readAtmos_multi(Atmosphere *atmos, Geometry *geometry,
  ...
  case 'H':
    ...
    for (k = 0; k < Ndep; k++)
      geometry->height[k] = dscale[k] * KM_TO_M;
    break;
  default:
  ...
  for (k = 0; k < Ndep; k++) {
    ...
    atmos->vturb[k] =
        atmos->vturb[k] * KM_TO_M * input.vturb_mult + input.vturb_add;
  ...
  if (k > 0 && k < Ndep) {
    ...
    for (k = 0; k < Ndep; k++) {
      ...
      nbaryon = (atmos->gravity * geometry->cmass[k] -
...
void convertScales(Atmosphere *atmos, Geometry *geometry) {
  ...
  case COLUMN_MASS:
    ...
    for (k = 1; k < Ndep; k++) {
      height[k] = height[k - 1] -
                  2.0 * (cmass[k] - cmass[k - 1]) / (rho[k - 1] + rho[k]);
  ...
  case TAU500:
    ...
    for (k = 1; k < Ndep; k++) {
      ...
      cmass[k] = cmass[k - 1] +
                 0.5 * (rho[k - 1] + rho[k]) * (height[k - 1] - height[k]);
    }
    break;
  case GEOMETRIC:
...
void getBoundary(Geometry *geometry) {
  ...
  case IRRADIATED:
    ...
    result &= xdr_vector(&xdrs, (char *)geometry->Itop[0],
                         spectrum.Nspect * geometry->Nrays, sizeof(double),
