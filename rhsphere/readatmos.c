...
void readAtmos(Atmosphere *atmos, Geometry *geometry) {
  ...
  getLine(atmos->fp_atmos, MULTI_COMMENT_CHAR, inputLine, exit_on_EOF = TRUE);
  Nread += sscanf(inputLine, "%d %d %d", &Nradius, &geometry->Ncore,
                  &geometry->Ninter);
  ...
  for (k = 0; k < Nradius; k++) {
    getLine(atmos->fp_atmos, MULTI_COMMENT_CHAR, inputLine, exit_on_EOF = TRUE);
    Nread = sscanf(inputLine, "%lf %lf %lf %lf %lf", &dscale[k], &atmos->T[k],
                   &atmos->ne[k], &geometry->vel[k], &atmos->vturb[k]);
  ...
  case 'T':
    ...
    for (k = 0; k < Nradius; k++)
      geometry->tau_ref[k] = POW10(dscale[k]);
    break;
  case 'H':
    geometry->scale = GEOMETRIC;
    for (k = 0; k < Nradius; k++)
      geometry->r[k] = dscale[k] * KM_TO_M;
    break;
  default:
  ...
  for (k = 0; k < Nradius; k++) {
    if (getLine(atmos->fp_atmos, MULTI_COMMENT_CHAR, inputLine,
      ...
      break;
    Nread = sscanf(inputLine, "%lf %lf %lf %lf %lf %lf", &atmos->nH[0][k],
                   &atmos->nH[1][k], &atmos->nH[2][k], &atmos->nH[3][k],
                   &atmos->nH[4][k], &atmos->nH[5][k]);
    checkNread(Nread, Nrequired = 6, routineName, checkPoint = 3);
  }

  if (k > 0 && k < Nradius) {
    sprintf(messageStr, "Reached end of input file %s before all data was read",
            input.atmos_input);
    Error(ERROR_LEVEL_2, routineName, messageStr);
  } else if (k == 0) {
    ...
    for (k = 0; k < Nradius; k++) {
      ...
      nbaryon =
          (atmos->gravity * geometry->cmass[k] + turbpress * atmos->ne[k]) /
              (KBOLTZMANN * atmos->T[k] + turbpress) -
          atmos->ne[k];
...
void convertScales(Atmosphere *atmos, Geometry *geometry) {
  ...
  case COLUMN_MASS:
    ...
    for (k = 1; k < Nradius; k++) {
      r[k] = r[k - 1] - 2.0 * (cmass[k] - cmass[k - 1]) / (rho[k - 1] + rho[k]);
      tau_ref[k] = tau_ref[k - 1] +
                   0.5 * (as->chi_c[k - 1] + as->chi_c[k]) * (r[k - 1] - r[k]);
    }
    break;
  case TAU500:
    r[0] = 0.0;
    ...
    for (k = 1; k < Nradius; k++) {
      ...
      cmass[k] = cmass[k - 1] + 0.5 * (rho[k - 1] + rho[k]) * (r[k - 1] - r[k]);
    }
    break;
  case GEOMETRIC:
...
void getBoundary(Geometry *geometry) {
  ...
  case IRRADIATED:
    ...
    result &= xdr_vector(&xdrs, (char *)geometry->Itop, spectrum.Nspect,
