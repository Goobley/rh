...
void solve_1D_plane(struct Atom *atom, Molecule *molecule, int NmaxIter,
                    double iterLimit) {
  const char routineName[] = "solve_1D_plane";
  register int k, n, nspect, niter;

  bool_t quiet, includesBB, to_obs, initialize, crosscoupling, boundbound,
      polarized, PRD_angle_dep, solve_Stokes, accel;
  enum FeautrierOrder F_order;
  int Nspace = atmos.Nspace, mu;
  double dPopsmax, *chi, *P, *Psi, *J, *Jdag, *S, xmu, wmu, *chi_as, *eta_as,
      vboundary[4] = {0.0, 0.0, 0.0, 0.0};
  ActiveSet *as;
    ...
    for (nspect = 0; nspect < spectrum.Nspect; nspect++) {
      ...
      if (polarized || PRD_angle_dep ||
        ...
        for (k = 0; k < Nspace; k++) {
          ...
          S[k] =
              (eta_as[k] + atmos.eta_c[k] + atmos.scatt[k] * Jdag[k]) / chi[k];
        }
        vboundary[1] = geometry.Itop[nspect];
        vboundary[3] = geometry.Icore[nspect];
        spectrum.I[nspect][mu] =
            Feautrier(Nspace, geometry.r, xmu, chi, S, vboundary,
                      F_order = STANDARD, P, TRUE);
