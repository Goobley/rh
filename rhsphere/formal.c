...
double Formal(int nspect, bool_t eval_operator, bool_t redistribute) {
  ...
  bool_t initialize, boundbound, PRD_angle_dep, to_obs, polarized, solve_Stokes;
...
void addtoRates_sphere(int nspect, Ray *ray, double *P, bool_t redistribute) {
  ...
  for (nact = 0; nact < atmos.Nactiveatom; nact++) {
    ...
    for (n = 0; n < as->Nactiveatomrt[nact]; n++) {
      ...
      if (Rij != NULL) {
        ...
        for (k = 0; k < ray->Ns; k++) {
          wmula =
              atom->rhth[nt].Vij[n][k] * atom->rhth[nt].wla[n][k] * ray->wmu[k];
          Rij[k] += P[k] * wmula;
...
void addtoGamma_sphere(int nspect, Ray *ray, double *P, double *Psi) {
    ...
    for (n = 0; n < as->Nactiveatomrt[nact]; n++) {
      ...
      for (k = 0; k < ray->Ns; k++) {
        atom->Gamma[ij][k] -= atom->rhth[nt].chi_up[i][k] * Psi[k] *
                              atom->rhth[nt].Uji_down[j][k] * ray->wmu[k];
      }
        ...
        if (jp == i) {
          for (k = 0; k < ray->Ns; k++) {
            atom->Gamma[ji][k] += atom->rhth[nt].chi_down[j][k] * Psi[k] *
                                  atom->rhth[nt].Uji_down[i][k] * ray->wmu[k];
          }
        }
      }
      if (input.Nthreads > 1)
  ...
  for (nact = 0; nact < atmos.Nactivemol; nact++) {
    ...
    for (n = 0; n < as->Nactivemolrt[nact]; n++) {
      ...
      for (k = 0; k < ray->Ns; k++) {
        if (molecule->n[k]) {
          wmula = molecule->rhth[nt].Vij[n][k] * molecule->rhth[nt].wla[n][k] *
                  ray->wmu[k];
          molecule->Gamma[ji][k] += P[k] * wmula;
          molecule->Gamma[ij][k] +=
              molecule->rhth[nt].gij[n][k] * (twohnu3_c2 + P[k]) * wmula;
        }
      }
      if (input.Nthreads > 1)
