      ...
      for (kr = 0; kr < metal->Ncont; kr++) {
        ...
        if (lambda <= lambdaEdge && lambda >= continuum->lambda[0]) {
          ...
          if (continuum->hydrogenic) {
            ...
            splineCoef(continuum->Nlambda, continuum->lambda, continuum->alpha);
...
flags passive_bb(double lambda, int nspect, int mu, bool_t to_obs, double *chi,
  ...
  int i, j, entry;
  double dlambda, phi, v, twohnu3_c2, hc, fourPI, hc_4PI, gij, Vij, **n;
  Atom *atom;
  AtomicLine *line;
  flags backgrflags;
      ...
      for (kr = 0; kr < atom->Nline; kr++) {
        ...
        if (fabs(lambda - line->lambda0) <= dlambda) {
          ...
          for (nc = 0; nc < line->Ncomponent; nc++) {
            for (k = 0; k < atmos.Nspace; k++) {
              v = (lambda - line->lambda0 - line->c_shift[nc]) * CLIGHT /
              ...
              if (line->Voigt)
                phi = Voigt(linelist[entry]->adamp[k], v, NULL, ARMSTRONG) *
                      line->c_fraction[nc];
              else
                phi = exp(-SQ(v));
