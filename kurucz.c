...
double RLKProfile(RLK_Line *rlk, int k, int mu, bool_t to_obs, double lambda,
                  double *phi_Q, double *phi_U, double *phi_V, double *psi_Q,
                  double *psi_U, double *psi_V);
...
void readKuruczLines(char *inputFile) {
  ...
  const double C =
      2.0 * PI * (Q_ELECTRON / EPSILON_0) * (Q_ELECTRON / M_ELECTRON) / CLIGHT;
  ...
  int Nline, Nread, Nrequired, checkPoint, hfs_i, hfs_j, gL_i, gL_j, iso_dl, i;
  unsigned int line_index;
  double lambda0, Ji, Jj, Grad, GStark, GvdWaals, pti, Ei, Ej, gf, lambda_air;
  size_t length;
  RLK_Line *rlk;
  ...
  for (line_index = 0; line_index < input.Nkurucz_files; line_index++) {
    ...
    if (input.kurucz_line_file_contents[line_index] != NULL) {
      ...
      strncpy(input.kurucz_line_file_name[line_index], filename,
              sizeof(filename));
    ...
    if (atmos.Nrlk == 0)
      atmos.rlk_lines = NULL;
    atmos.rlk_lines = (RLK_Line *)realloc(
        atmos.rlk_lines, (Nline + atmos.Nrlk) * sizeof(RLK_Line));
    ...
    while (sgets(inputLine, RLK_RECORD_LENGTH + 1, &fp_linelist) != NULL) {
      if (*inputLine != *commentChar) {
        ...
        Nread = sscanf(inputLine, "%lf %lf %s %lf", &lambda_air, &gf,
                       (char *)&elem_code, &Ei);
...
flags rlk_opacity(double lambda, int nspect, int mu, bool_t to_obs, double *chi,
  ...
  double dlamb_wing, *pf, dlamb_char, hc_la, ni_gi, nj_gj, lambda0, kT,
      Bijhc_4PI, twohnu3_c2, hc, fourPI, hc_4PI, *eta_Q, *eta_U, *eta_V, eta_l,
      *chi_Q, *chi_U, *chi_V, chi_l, *chip_Q, *chip_U, *chip_V, phi, phi_Q,
      phi_U, phi_V, psi_Q, psi_U, psi_V, epsilon, C, C2_atom, C2_ion, C3, dE, x;
  Atom *metal;
  AtomicLine *line;
  Element *element;
  RLK_Line *rlk;
  ...
  Nblue = Nwhite;
  while (atmos.rlk_lines[Nblue].lambda0 + dlamb_char > lambda && Nblue > 0)
    Nblue--;
  Nred = Nwhite;
  ...
  for (n = Nblue; n <= Nred; n++) {
    ...
    if (fabs(rlk->lambda0 - lambda) <= dlamb_char) {
      ...
      if ((rlk->stage < element->Nstage - 1) && element->abundance_set) {
        ...
        if ((metal = element->model) != NULL) {
          ...
          for (kr = 0; kr < metal->Nline; kr++) {
            line = metal->line + kr;
            dlamb_wing =
                line->lambda0 * line->qwing * (atmos.vmicro_char / CLIGHT);
      ...
      if (contributes) {
        ...
        Linear(atmos.Npf, atmos.Tpf, element->pf[rlk->stage], atmos.Nspace,
               atmos.T, pf, hunt = TRUE);

        for (k = 0; k < atmos.Nspace; k++) {
          phi = RLKProfile(rlk, k, mu, to_obs, lambda, &phi_Q, &phi_U, &phi_V,
                           &psi_Q, &psi_U, &psi_V);
...
double RLKProfile(RLK_Line *rlk, int k, int mu, bool_t to_obs, double lambda,
  ...
  double v, phi_sm, phi_sp, phi_pi, psi_sm, psi_sp, psi_pi, adamp, vB, H, F, sv,
      phi_sigma, phi_delta, sign, sin2_gamma, phi, psi_sigma, psi_delta, vbroad,
      vtherm, GvdW, *np;
  Element *element;
  ...
  for (Ml = -Jl; Ml <= Jl; Ml++) {
    for (Mu = -Ju; Mu <= Ju; Mu++) {
      if (fabs(Mu - Ml) <= 1.0) {
        ...
        if (zm->q[n] == 1)
          g_eff += zm->shift[n] * zm->strength[n];
        n++;
      }
    }
  }
...
bool_t RLKdeterminate(char *labeli, char *labelj, RLK_Line *rlk) {
  ...
  if (words[0]) {
    ...
    Nread =
        sscanf(words[count - 1] + length - 2, "%d%1s", &multiplicity, orbit);
    free(words);
    if (Nread != 2 || !isupper(orbit[0]))
      return FALSE;
...
void getUnsoldcross(RLK_Line *rlk) {
  ...
  vrel35_H = pow(8.0 * KBOLTZMANN / (PI * AMU * element->weight) *
                     (1.0 + element->weight / atmos.H->weight),
                 0.3);
  vrel35_He = pow(8.0 * KBOLTZMANN / (PI * AMU * element->weight) *
                      (1.0 + element->weight / He->weight),
                  0.3);

  C625 = pow(2.5 * (SQ(Q_ELECTRON) / FOURPIEPS0) * (ABARH / FOURPIEPS0) * 2 *
                 PI * SQ(Z * RBOHR) / HPLANCK * deltaR,
             0.4);
