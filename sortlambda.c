...
void SortLambda() {
  ...
  if (strcmp(input.wavetable_input, "none")) {
    if (input.wavetable == NULL) {
      ...
      result &= xdr_vector(&xdrs, (char *)wavetable, Nwave, sizeof(double),
  ...
  if (atmos.Nactivemol > 0) {
    atmos.activemols =
        (Molecule **)malloc(atmos.Nactivemol * sizeof(Molecule *));
    for (n = 0; n < atmos.Nmolecule; n++) {
      molecule = &atmos.molecules[n];
      if (molecule->active)
  ...
  spectrum.lambda =
      (double *)realloc(spectrum.lambda, spectrum.Nspect * sizeof(double));
  ...
  for (nspect = 0; nspect < spectrum.Nspect; nspect++) {
    ...
    if (atmos.Nactiveatom > 0) {
      ...
      as->art = (AtomicTransition **)malloc(atmos.Nactiveatom *
                                            sizeof(AtomicTransition *));

      for (nact = 0; nact < atmos.Nactiveatom; nact++) {
        as->Nactiveatomrt[nact] = 0;
        as->art[nact] = (AtomicTransition *)malloc(N_MAX_OVERLAP *
                                                   sizeof(AtomicTransition));
      }
    } else {
    ...
    if (atmos.Nactivemol > 0) {
      ...
      as->mrt =
          (MolTransition **)malloc(atmos.Nactivemol * sizeof(MolTransition *));

      for (nact = 0; nact < atmos.Nactivemol; nact++) {
        as->Nactivemolrt[nact] = 0;
        as->mrt[nact] =
            (MolTransition *)malloc(N_MAX_OVERLAP * sizeof(MolTransition));
      }
    } else {
  ...
  for (nact = 0; nact < atmos.Nactiveatom; nact++) {
    ...
    for (kr = 0; kr < atom->Ncont; kr++) {
      ...
      Hunt(spectrum.Nspect, spectrum.lambda, continuum->lambda[0],
           &continuum->Nblue);
      ...
      if (continuum->hydrogenic) {
        ...
        continuum->alpha = (double *)realloc(
            continuum->alpha, continuum->Nlambda * sizeof(double));
        ...
        for (la = 0; la < continuum->Nlambda; la++) {
          continuum->alpha[la] =
              continuum->alpha0 * Gaunt_bf(continuum->lambda[la], n_eff, Z) /
              gbf_0 * CUBE(continuum->lambda[la] / continuum->lambda0);
        }
      } else {
    ...
    for (kr = 0; kr < atom->Nline; kr++) {
      ...
      Hunt(spectrum.Nspect, spectrum.lambda, line->lambda[line->Nlambda - 1],
           &Nred);
  ...
  for (nact = 0; nact < atmos.Nactivemol; nact++) {
    ...
    for (kr = 0; kr < molecule->Nrt; kr++) {
      ...
      Hunt(spectrum.Nspect, spectrum.lambda, mrt->lambda[mrt->Nlambda - 1],
           &Nred);
  ...
  for (nspect = 0; nspect < spectrum.Nspect; nspect++) {
    ...
    for (nact = 0; nact < atmos.Nactiveatom; nact++) {
      if (as->Nactiveatomrt[nact] > 0) {
        as->art[nact] = (AtomicTransition *)realloc(
            as->art[nact], as->Nactiveatomrt[nact] * sizeof(AtomicTransition));

        as->lower_levels[nact] = (int *)realloc(as->lower_levels[nact],
                                                as->Nlower[nact] * sizeof(int));
        as->upper_levels[nact] = (int *)realloc(as->upper_levels[nact],
    ...
    for (nact = 0; nact < atmos.Nactivemol; nact++) {
      if (as->Nactivemolrt[nact] > 0) {
        as->mrt[nact] = (MolTransition *)realloc(
            as->mrt[nact], as->Nactivemolrt[nact] * sizeof(MolTransition));
      }
    }
  }
