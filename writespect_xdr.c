...
void writeSpectrum(Spectrum *spectrum) {
  ...
  if (!strcmp(input.spectrum_output, "none"))
    return;

  if ((fp_spectrum = fopen(input.spectrum_output, "w")) == NULL) {
    sprintf(messageStr, "Unable to open output file %s", input.spectrum_output);
  ...
  if (spectrum->updateJ && !input.limit_memory) {
    if ((spectrum->fd_J = open(input.JFile, O_WRONLY | O_CREAT, PERMISSIONS)) ==
        -1) {
