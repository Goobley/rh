...
void writeMULTIatmos(Geometry *geometry, Atmosphere *atmos, char *modelName) {
  ...
  if (modelName != NULL) {
    ...
    fprintf(fp_MULTI, "%s %s%s\n  %s\n", COMMENT_CHAR, atmos->ID, COMMENT_CHAR,
            modelName);
  } else
    fp_MULTI = stdout;
  ...
  fprintf(fp_MULTI, "%s\n%s log g [cm s^-2]\n %7.4f\n", COMMENT_CHAR,
          COMMENT_CHAR, log10(atmos->gravity));
  ...
  case COLUMN_MASS:
    fprintf(fp_MULTI,
            "%slg column Mass     Temperature        Ne         V"
            "              Vturb\n",
            COMMENT_CHAR);
  ...
  for (k = 0; k < geometry->Ndep; k++) {
    ...
    fprintf(fp_MULTI, "%17.8E %14.6E %14.6E %14.6E %14.6E\n", height,
            atmos->T[k], atmos->ne[k], geometry->vel[k], vturb);
  }
  ...
  if (atmos->B != NULL) {
    ...
    result &= xdr_vector(&xdrs, (char *)atmos->B, atmos->Nspace, sizeof(double),
