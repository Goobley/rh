...
void writeModelAtom(Atom *atom, FILE *fp_out) {
  ...
  fprintf(fp_out, "  %3d    %4d     %3d       %2d\n\n", atom->Nlevel,
          atom->Nline, atom->Ncont, atom->Nfixed);
  ...
  for (i = 0; i < atom->Nlevel; i++) {
    ...
    fprintf(fp_out, " %10.3f  %6.2f   '%-20s'    %1d     %3d\n", Ecm,
            atom->g[i], atom->label[i], atom->stage[i], i);
  }
      ...
      for (kf = 0; kf < atom->Nfixed; kf++) {
        ...
        if ((fixed->type == ATOMIC_LINE) && (fixed->j == j)) {
          line->Grad += C / SQ(fixed->lambda0) *
                        (atom->g[fixed->i] / atom->g[fixed->j]) *
                        fixed->strength;
        }
      }
    }
    f = line->Aji * (atom->g[j] / atom->g[i]) * SQ(line->lambda0 * NM_TO_M) / C;
    fprintf(fp_out, " %3d  %3d  %9.3E  %s    %3d    %5s  %4.1f  %6.1f", j, i, f,
            shapeStr, line->Nlambda, symmStr, line->qcore, line->qwing);
    fprintf(fp_out, "   %-7s  %5.3f  %5.3f  %5.3f  %5.3f  %8.2E  %9.2E\n",
            vdwStr, line->cvdWaals[0], line->cvdWaals[1], line->cvdWaals[2],
            line->cvdWaals[3], line->Grad, line->cStark);
  }
  ...
  for (kr = 0; kr < atom->Ncont; kr++) {
    ...
    fprintf(fp_out, " %2d  %2d     %10.3E      %3d       %s     %8.3f\n", j, i,
            continuum->alpha0, continuum->Nlambda, shapeStr,
            continuum->lambda[0]);
  ...
  for (kf = 0; kf < atom->Nfixed; kf++) {
    ...
    case TRAD_ATMOSPHERIC:
      strcpy(optionStr, "TRAD_ATMOSPHERIC");
      break;
    case TRAD_PHOTOSPHERIC:
      strcpy(optionStr, "TRAD_PHOTOSPHERIC");
      break;
    case TRAD_CHROMOSPHERIC:
      strcpy(optionStr, "TRAD_CHROMOSPHERIC");
      break;
    }
    fprintf(fp_out, " %2d  %2d     %10.3E      %6.1f     %s\n", fixed->j,
            fixed->i, fixed->strength, fixed->Trad, optionStr);
  }
}
...
void writeCollisions(FILE *atomFile, Atom *atom, FILE *fp_out) {
  ...
  while ((getLine(atomFile, MULTI_COMMENT_CHAR, input, exit_on_EOF = FALSE) !=
          EOF)) {
