...
void main(int argc, char *argv[]) {
  ...
  char input[MAX_LINE_SIZE], parity[2], config1[10], config2[10], state[4],
      orbital[2], spectrumID[5];
  bool_t exit_on_EOF, readphoto = FALSE, hunt, lines = FALSE;
  int Nlevel, Nl, nread, itb, *islp, *ilv, ln, ll, multiplicity, islp_p, ilv_p,
      i_p, nz, ne, nprint, Nlamb, stage, i_l, islp_l, ilv_l, jslp_l, jlv_l,
      Nrad, ntoken;
  double gi, *te, e, rl, abundance, weight, Eion, *alpha, *E, hERc, Econt, fdum,
      gf, f, lambda0, C, Ecorr, E0;
  ...
  Nl = 0;
  while (getLine(fp_energy, "=", input, exit_on_EOF = FALSE) != EOF)
    Nl++;
  ...
  case 0:
    ...
    break;
  case 1:
    strcpy(spectrumID, "II");
    break;
  case 2:
    strcpy(spectrumID, "III");
    break;
  case 3:
    strcpy(spectrumID, "IV");
    break;
  case 4:
    strcpy(spectrumID, "V");
    break;
  ...
  for (i = 0; i < atom.Nlevel - 1; i++) {
    ...
    switch (ntoken) {
    case 10:
      sscanf(input, "%d %d %d %d %d %s %lf %lf %lf %lf", &itb, islp + i,
             ilv + i, &ln, &ll, config1, &gi, te + i, &e, &rl);
      break;
    case 11:
      sscanf(input, "%d %d %d %d %d %s %s %lf %lf %lf %lf", &itb, islp + i,
             ilv + i, &ln, &ll, config1, config2, &gi, te + i, &e, &rl);
      break;
    case 12:
      sscanf(input, "%d %d %d %d %d %s %s %s %lf %lf %lf %lf", &itb, islp + i,
             ilv + i, &ln, &ll, config1, config2, state, &gi, te + i, &e, &rl);
      break;
    default:;
    }
    atom.g[i] = gi;
    ...
    *(orbital + 1) = '\0';
    ...
    switch (ntoken) {
    case 10:
      nprint = sprintf(atom.label[i], "%s %s %s %1d%s%s", atom.ID, spectrumID,
                       config1, multiplicity, orbital, parity);
      break;
    case 11:
      nprint =
          sprintf(atom.label[i], "%s %s %s %s %1d%s%s", atom.ID, spectrumID,
                  config1, config2, multiplicity, orbital, parity);
      break;
    case 12:
      nprint =
          sprintf(atom.label[i], "%s %s %s %s %s %1d%s%s", atom.ID, spectrumID,
                  config1, config2, state, multiplicity, orbital, parity);
      break;
    default:;
    }
  ...
  if (lines) {
    ...
    while (getLine(fp_lines, "=", input, exit_on_EOF = FALSE) != EOF) {
      nread = sscanf(input, "%d %d %d %d %d %lf %lf", &i_l, &islp_l, &ilv_l,
                     &jslp_l, &jlv_l, &gf, &fdum);
        ...
        if ((islp[i] == islp_l) && (ilv[i] == ilv_l)) {
          for (j = 0; j < atom.Nlevel; j++) {
            if ((islp[j] == jslp_l) && (ilv[j] == jlv_l)) {
              ...
              lambda0 =
                  (HPLANCK * CLIGHT) / (atom.E[line->j] - atom.E[line->i]);
              line->Aji =
                  C / SQ(lambda0) * (atom.g[line->i] / atom.g[line->j]) * f;
  ...
  if (readphoto) {
    while (getLine(fp_photo, "=", input, exit_on_EOF = FALSE) != EOF) {
      ...
      nread = sscanf(input, "%d %d %d %d %d %lf %d", &i_p, &nz, &ne, &islp_p,
                     &ilv_p, &Eion, &Nlamb);
      ...
      for (i = 0; i < atom.Nlevel; i++) {
        if ((islp[i] == islp_p) && (ilv[i] == ilv_p)) {
          ...
          for (la = 0; la < continuum->Nlambda; la++) {
            continuum->lambda[la] =
                hERc / ((E[(continuum->Nlambda - 1) - la] + Ecorr) * NM_TO_M);
            continuum->alpha[la] =
                alpha[(continuum->Nlambda - 1) - la] * MEGABARN_TO_M2;
          }
          ...
          Linear(continuum->Nlambda, E, alpha, 1, &Eion, &(continuum->alpha0),
                 hunt = FALSE);
  ...
  atom.line = (AtomicLine *)realloc(atom.line, atom.Nline * sizeof(AtomicLine));
  atom.continuum = (AtomicContinuum *)realloc(
      atom.continuum, atom.Ncont * sizeof(AtomicContinuum));
