...
void impactParam(AtomicLine *line, bool_t flag, int Ntemp, double *temp,
                 double *CE);
...
void main(int argc, char *argv[]) {
  ...
  double temp[NTEMP] = {1264.9,   2249.4,   4000.0,   7113.1,   12649.1,
                        22493.6,  40000.0,  71131.1,  126491.0, 224936.0,
                        400000.0, 711312.0, 1264910.0};
  FILE *fp_out;
  Atom atom;
...
void crossSection(Atom *atom, int Ntemp, double *temp, FILE *fp_out) {
      ...
      if (atom->stage[j] == atom->stage[i]) {
        ...
        if (atom->stage[i] == 0) {
          ...
          for (k = 0; k < Ntemp; k++) {
            ...
            fprintf(fp_out, "%9.3E%2s", Omega[k],
                    (k == Ntemp - 1) ? methodStr : "  ");
          }
        }
      }
  ...
  for (kr = 0; kr < atom->Ncont; kr++) {
    ...
    for (k = 0; k < Ntemp; k++) {
      CI[k] =
          C0 / temp[k] * alpha0 * gbar_i * ((KBOLTZMANN * temp[k]) / deltaE);
...
double findbeta1(double righthand, double *zeta1) {
  ...
  while ((delta = 1.0 - beta1 / beta2) > DELTA) {
    ...
    if ((y = square(Bessel_K0(beta)) + square(Bessel_K1(beta)) - righthand) >
        0.0)
      beta1 = beta;
    else
      beta2 = beta;
