...
void main(int argc, char *argv[]) {
  ...
  char inputLine[MAX_REC_LENGTH + 1], *commentChar = COMMENT_CHAR,
                                      atomID[ATOM_ID_WIDTH + 1], spectrum[6],
                                      label[25], *ptr, term[3], Jstr[5];
  ...
  while (fgets(inputLine, MAX_REC_LENGTH + 1, fp_NIST) != NULL) {
    if (*inputLine != *commentChar) {
      Nread = sscanf(inputLine, "%2s %s %s", atomID, spectrum, label);
      UpperCase(atomID);
      ...
      sprintf(nl->label, "%2s %s %-9.9s %2s%1s", atomID, spectrum, label, term,
              (nl->parity) ? "O" : "E");
  ...
  for (i = 0; i < Nlevel - 1; i++) {
    ...
    if (strncmp(levels[i + 1].label, label, ATOM_LABEL_WIDTH) != 0) {
      fprintf(stdout, " %10.3f  %6.2f   '%-20s'    %1d     %3d\n", E0 + E / g,
              g, levels[i].label, levels[i].stage, n);
      g = E = 0.0;
      n++;
