...
void MULTIatom(FILE *atomFile, Atom *atom) {
  ...
  getLine(atomFile, MULTI_COMMENT_CHAR, input, exit_on_EOF = TRUE);
  Nread = sscanf(input, "%d %d %d %d", &atom->Nlevel, &atom->Nline,
  ...
  for (i = 0; i < atom->Nlevel; i++) {
    ...
    getLine(atomFile, MULTI_COMMENT_CHAR, input, exit_on_EOF = TRUE);
    Nread = sscanf(input, "%lf %lf '%20c' %d", &atom->E[i], &atom->g[i],
  ...
  for (kr = 0; kr < atom->Nline; kr++) {
    ...
    getLine(atomFile, MULTI_COMMENT_CHAR, input, exit_on_EOF = TRUE);
    Nread = sscanf(input, "%d %d %lf %d %lf %lf %d %lf %lf %lf", &j, &i, &f,
                   &line->Nlambda, &line->qwing, &line->qcore, &iw, &line->Grad,
                   &line->cvdWaals[0], &line->cStark);
  ...
  for (kr = 0; kr < atom->Ncont; kr++) {
    ...
    getLine(atomFile, MULTI_COMMENT_CHAR, input, exit_on_EOF = TRUE);
    Nread = sscanf(input, "%d %d %lf %d %lf", &j, &i, &continuum->alpha0,
    ...
    if (lambdaMin < 0.0) {
      ...
      for (la = 0; la < continuum->Nlambda; la++) {
        ...
        getLine(atomFile, MULTI_COMMENT_CHAR, input, exit_on_EOF = TRUE);
        sscanf(input, "%lf %lf", continuum->lambda + lap,
               continuum->alpha + lap);
  ...
  atom->ft = (FixedTransition *)malloc(atom->Nfixed * sizeof(FixedTransition));
  for (kf = 0; kf < atom->Nfixed; kf++) {
    fixed = atom->ft + kf;

    getLine(atomFile, MULTI_COMMENT_CHAR, input, exit_on_EOF = TRUE);
    Nread = sscanf(input, "%d %d %d %lf %lf %d", &j, &i, &phot, &A0,
                   &fixed->Trad, &Trad_option);
