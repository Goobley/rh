...
void main(int argc, char *argv[]) {
  ...
  int Nft, Nlamb, nread;
  double hERc, *E, *lambda, *alpha, *Eft, *lft, dE, dlamb, fdum, lambdaMin,
      cutoff;
  double *aft;
  FILE *fp_script;
  struct Atom atom;
  ...
  while (getLine(fp_script, "*", inputLine, exit_on_EOF = FALSE) != EOF) {
    nread = sscanf(inputLine, "'%20c' %s %d %f", label, keyword, &Nlamb, &fdum);
    ...
    for (kr = atom.Nline; kr < (atom.Nline + atom.Ncont); kr++) {
      ...
      if (strstr(label, atom.label[cont->i])) {
        ...
        if (strstr(keyword, "FFT")) {
          ...
          for (la = 1; la < Nft; la++)
            Eft[la] = Eft[la - 1] + dE;
          Linear(cont->Nlamb, E, cont->alpha, Nft, Eft, aft, hunt = TRUE);
          ...
          cont->alpha = (double *)realloc(cont->alpha, Nlamb * sizeof(double));
          cont->lambda =
          ...
          Linear(Nft, lft, aft, Nlamb, cont->lambda, cont->alpha, hunt = TRUE);
          ...
          for (la = 0; la < cont->Nlamb; la++) {
            cont->alpha[la] =
                cont->alpha0 * CUBE(cont->lambda[Nlamb - 1] / cont->lambda[la]);
          }
          ...
          for (la = 1; la < Nlamb; la++)
            cont->lambda[la] = cont->lambda[la - 1] + dlamb;
          Linear(cont->Nlamb, lambda, alpha, Nlamb, cont->lambda, cont->alpha,
                 hunt = TRUE);
