...
void main(int argc, char *argv[]) {
  ...
  struct Option theOpt[] = {{"-help", FALSE, 2, "Print this message          "},
  ...
  result &= xdr_vector(&xdrs, (char *)lambda, Nlambda, sizeof(double),
                       (xdrproc_t)xdr_double);

  if (!result) {
    sprintf(messageStr, "Unable to read from input file %s", argv[argc - 1]);
  }
  ...
  if (to_air)
    vacuum_to_air(Nlambda, lambda, lambda);
  ...
  if (showfull) {
    for (i = 0; i < Nlambda - 5; i += 5) {
      printf(format, lambda[i], lambda[i + 1], lambda[i + 2], lambda[i + 3],
             lambda[i + 4]);
    }
