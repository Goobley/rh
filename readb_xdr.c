...
bool_t readB(Atmosphere *atmos) {
  ...
  if (input.xdr_endian) {
    ...
    result &=
        (fread(atmos->B, sizeof(double), recordsize, fp_stokes) == recordsize);
    result &= (fread(atmos->gamma_B, sizeof(double), recordsize, fp_stokes) ==
               recordsize);
    result &= (fread(atmos->chi_B, sizeof(double), recordsize, fp_stokes) ==
               recordsize);
  }
