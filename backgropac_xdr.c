...
void backgrOpac(int Nlambda, double *lambda) {
  ...
  for (nspect = 0; nspect < Nlambda; nspect++) {
    ...
    sprintf(format, "bopac%%%1.1d.3f", (int)log10(lambda[nspect]) + 5);
    sprintf(contribution_out, format, lambda[nspect]);
    if ((fp_out = fopen(contribution_out, "w")) == NULL) {
      sprintf(messageStr, "Unable to open output file %s", contribution_out);
    ...
    write_contrib_xdr(&xdrs, "THOMSON", type = SCATTERING, atmos.Nspace,
                      thomson);

    /* --- Negative hydrogen ion, bound-free and free-free -- ------- */

    if (Hminus_bf(lambda[nspect], chi, eta)) {
      write_contrib_xdr(&xdrs, "HMINUS_BF", type = ABSORPTION, atmos.Nspace,
                        chi);
    }
    if (Hminus_ff(lambda[nspect], chi)) {
      write_contrib_xdr(&xdrs, "HMINUS_FF", type = ABSORPTION, atmos.Nspace,
                        chi);
    }
    /* --- Bound-free opacities from OH and CH molecules -- --------- */

    if (OH_bf_opac(lambda[nspect], chi, eta)) {
      write_contrib_xdr(&xdrs, "OH_BF", type = ABSORPTION, atmos.Nspace, chi);
    }
    if (CH_bf_opac(lambda[nspect], chi, eta)) {
      write_contrib_xdr(&xdrs, "CH_BF", type = ABSORPTION, atmos.Nspace, chi);
    }

    /* --- Neutral Hydrogen Bound-Free and Free-Free --  ------------ */

    if (Hydrogen_bf(lambda[nspect], chi, eta)) {
      write_contrib_xdr(&xdrs, "HYDROGEN_BF", type = ABSORPTION, atmos.Nspace,
                        chi);
    }
    Hydrogen_ff(lambda[nspect], chi);
    write_contrib_xdr(&xdrs, "HYDROGEN_FF", type = ABSORPTION, atmos.Nspace,
                      chi);

    /* --- Rayleigh scattering by neutral hydrogen --  -------------- */

    if (Rayleigh(lambda[nspect], atmos.H, scatt)) {
      write_contrib_xdr(&xdrs, "RAYLEIGH_H", type = SCATTERING, atmos.Nspace,
                        scatt);
    }
    /* --- Rayleigh scattering by neutral helium --    -------------- */

    if (He && Rayleigh(lambda[nspect], He, scatt)) {
      write_contrib_xdr(&xdrs, "RAYLEIGH_HE", type = SCATTERING, atmos.Nspace,
                        scatt);
    }
    /* --- Absorption by H + H^+ (referred to as H2plus free-free) -- */

    if (H2plus_ff(lambda[nspect], chi))
      write_contrib_xdr(&xdrs, "H2PLUS_FF", type = ABSORPTION, atmos.Nspace,
                        chi);
    ...
    if (Rayleigh_H2(lambda[nspect], scatt)) {
      write_contrib_xdr(&xdrs, "RAYLEIGH_H2", type = SCATTERING, atmos.Nspace,
                        scatt);
    }
    if (H2minus_ff(lambda[nspect], chi)) {
      write_contrib_xdr(&xdrs, "H2MINUS_FF", type = ABSORPTION, atmos.Nspace,
                        chi);
    }
    ...
    for (n = 1; n < atmos.Natom; n++) {
      ...
      write_contrib_xdr(&xdrs, metalID, type = ABSORPTION, atmos.Nspace, chi);
    }
    xdr_destroy(&xdrs);
    fclose(fp_out);
  }
...
bool_t write_contrib_xdr(XDR *xdrs, char *label, enum opac_type type,
  ...
  result &= xdr_vector(xdrs, (char *)scatt, Nspace, sizeof(double),
