...
void Profile(AtomicLine *line) {
  ...
  if (input.limit_memory) {
    sprintf(filename,
            (atom->ID[1] == ' ') ? "profile.%.1s_%d-%d.dat"
                                 : "profile.%.2s_%d-%d.dat",
            atom->ID, line->j, line->i);
    if ((line->fd_profile = open(filename, O_RDWR | O_CREAT, PERMISSIONS)) ==
        -1) {
  ...
  if (atmos.moving || (line->polarizable && (input.StokesMode > FIELD_FREE))) {
            ...
            for (k = 0; k < atmos.Nspace; k++) {
              ...
              for (n = 0; n < line->Ncomponent; n++) {
                ...
                for (nz = 0; nz < zm->Ncomponent; nz++) {
                  H = Voigt(adamp[k], vk - zm->shift[nz] * vB[k], &F, HUMLICEK);
                ...
                phi_delta =
                    0.5 * phi_pi * line->c_fraction[n] - 0.25 * phi_sigma;

                phi[k] += (phi_delta * sin2_gamma + 0.5 * phi_sigma) * sv[k];
                phi_Q[k] += sign * phi_delta * sin2_gamma *
                            atmos.cos_2chi[mu][k] * sv[k];
                phi_U[k] +=
                    phi_delta * sin2_gamma * atmos.sin_2chi[mu][k] * sv[k];
                phi_V[k] += sign * 0.5 * (phi_sp - phi_sm) *
                            atmos.cos_gamma[mu][k] * sv[k];

                if (input.magneto_optical) {
                  psi_sigma = (psi_sp + psi_sm) * line->c_fraction[n];
                  psi_delta =
                      0.5 * psi_pi * line->c_fraction[n] - 0.25 * psi_sigma;

                  psi_Q[k] += sign * psi_delta * sin2_gamma *
                              atmos.cos_2chi[mu][k] * sv[k];
                  psi_U[k] +=
                      psi_delta * sin2_gamma * atmos.sin_2chi[mu][k] * sv[k];
                  psi_V[k] += sign * 0.5 * (psi_sp - psi_sm) *
                              atmos.cos_gamma[mu][k] * sv[k];
                }
              }
    ...
    for (la = 0; la < line->Nlambda; la++) {
      ...
      for (k = 0; k < atmos.Nspace; k++) {
        for (n = 0; n < line->Ncomponent; n++) {
          vk = (line->lambda[la] - line->lambda0 - line->c_shift[n]) * CLIGHT /
               (line->lambda0 * atom->vbroad[k]);
          phi[k] += Voigt(adamp[k], vk, NULL, ARMSTRONG) * line->c_fraction[n] /
                    (SQRTPI * atom->vbroad[k]);
        }
        line->wphi[k] += phi[k] * wlamu;
      }
      if (input.limit_memory)
        writeProfile(line, la, phi);
    }
  }
  /* --- Store the inverse of the profile normalization -- ---------- */

  for (k = 0; k < atmos.Nspace; k++)
    line->wphi[k] = 1.0 / line->wphi[k];
...
void MolecularProfile(MolecularLine *mrt) {
  ...
  double *adamp, *v, vk, **v_los, wlamu, *sv, *phi_down, *phi_up, wlambda;
  ...
  if (atmos.moving) {
    for (la = 0; la < mrt->Nlambda; la++) {
      ...
      for (mu = 0; mu < atmos.Nrays; mu++) {
        ...
        for (k = 0; k < atmos.Nspace; k++) {
          phi_down[k] =
              Voigt(adamp[k], v[k] - v_los[mu][k], NULL, ARMSTRONG) * sv[k];
          phi_up[k] =
              Voigt(adamp[k], v[k] + v_los[mu][k], NULL, ARMSTRONG) * sv[k];
