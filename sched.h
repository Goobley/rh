...
*/
SCHED_API void scheduler_start(struct scheduler*, void *memory, void (*thread_init_fn)(void*,sched_uint));
...
struct sched_thread_args {
    ...
    void (*thread_init_fn)(void*,sched_uint);
...
sched_tasking_thread_f(void *pArgs)
{
    ...
    sched_call(args.thread_init_fn, NULL, thread_num);
...
SCHED_API void
scheduler_start(struct scheduler *s, void *memory, void(*init_fn)(void*,sched_uint))
{
    sched_uint i = 0;
    SCHED_ASSERT(s);
    SCHED_ASSERT(memory);
    ...
    s->args[0].thread_init_fn = init_fn;
    if (init_fn)
    {
        init_fn(NULL, 0);
    }
    ...
    for (i = 1; i < s->threads_num; ++i) {
        ...
        s->args[i].thread_init_fn = init_fn;
