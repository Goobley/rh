...
void LTEpops(Atom *atom, bool_t Debeye) {
  ...
  for (k = 0; k < Nspace; k++) {
    if (Debeye)
      dEion = c2 * sqrt(atmos.ne[k] / atmos.T[k]);
...
void LTEpops_elem(Element *element) {
  ...
  Linear(atmos.Npf, atmos.Tpf, element->pf[0], atmos.Nspace, atmos.T, Uk,
         hunt = TRUE);

  for (i = 1; i < element->Nstage; i++) {
    Linear(atmos.Npf, atmos.Tpf, element->pf[i], atmos.Nspace, atmos.T, Ukp1,
           hunt = TRUE);

    for (k = 0; k < atmos.Nspace; k++) {
      element->n[i][k] =
          element->n[i - 1][k] * CT_ne[k] *
          exp(Ukp1[k] - Uk[k] -
...
void LTEmolecule(Molecule *molecule) {
  ...
  for (v = 0; v < molecule->Nv; v++) {
    for (J = 0; J < molecule->NJ; J++) {
      gJ = 2 * J + 1;
      for (k = 0; k < atmos.Nspace; k++)
        molecule->pfv[v][k] += gJ * exp(-E[v][J] / (KBOLTZMANN * atmos.T[k]));
    }
...
void SetLTEQuantities(void) {
  ...
  for (n = 0; n < atmos.Natom; n++) {
    ...
    if (atom->active) {
      ...
      //        atom->offset_coll, and somehow read the file again! (filename
      //        gone)
