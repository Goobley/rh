...
/* --- Reads collisional data from atomic data file and computes
       ...
       Format is similar to MULTI's routine GENCOL:
         ...
         CR        -->  Collisional recombination (Added by Hsiao-hsuan, RECO in
 MULTI)
         ...
         SUMMERS   -->  Scaling factor for density-dependent dielectronic
 recombination according to Summers (1974). Default is 0, use 1 for full Summers
 coefficients.
...
double ar85cea(int i, int j, int k, struct Atom *atom) {
  ...
  if (!strcmp(cseq, "MG") || !strcmp(cseq, "AL") || !strcmp(cseq, "SI") ||
    ...
    if (!strcmp(cseq, "MG"))
      iea = 10.3 * pow(zz - 10.0, 1.52);
    if (!strcmp(cseq, "AL"))
      iea = 18.0 * pow(zz - 11.0, 1.33);
    if (!strcmp(cseq, "SI"))
      iea = 18.4 * pow(zz - 12.0, 1.36);
    if (!strcmp(cseq, "P"))
      iea = 23.7 * pow(zz - 13.0, 1.29);
    if (!strcmp(cseq, "S"))
      iea = 40.1 * pow(zz - 14.0, 1.1);
...
int atomnr(char ID[ATOM_ID_WIDTH + 1]) {
  ...
  while (strcmp(atmos.elements[i].ID, ID))
    i++;
...
void CollisionRate(struct Atom *atom, char *fp_atom) {
  ...
  int nitem, i1, i2, i, j, ij, ji, Nlevel = atom->Nlevel, Nitem, status;
  long Nspace = atmos.Nspace;
  fpos_t collpos;
  double dE, C0, *T, *coeff, *C, Cdown, Cup, gij, *np, xj, fac, fxj;
  int Ncoef, Nrow;
  double **cdi, **badi;
  double acolsh, tcolsh, aradsh, xradsh, adish, bdish, t0sh, t1sh, summrs, tg,
      cdn, cup;
  ...
  T = coeff = NULL;
  while ((status = getLineString(&fp_atom, COMMENT_CHAR, inputLine,
    ...
    if (!strcmp(keyword, "TEMP")) {
      ...
      if (Nrow > MSHELL) {
        sprintf(messageStr, "Nrow: %i greater than mshell %i", Nrow, MSHELL);
    ...
    if (!strcmp(keyword, "OMEGA")) {
      ...
      for (k = 0; k < Nspace; k++) {
        Cdown = C0 * atmos.ne[k] * C[k] / (atom->g[j] * sqrt(atmos.T[k]));
        atom->C[ij][k] += Cdown;
      ...
      for (k = 0; k < Nspace; k++) {
        Cup = C[k] * atmos.ne[k] * exp(-dE / (KBOLTZMANN * atmos.T[k])) *
              sqrt(atmos.T[k]);
        atom->C[ji][k] += Cup;
      ...
      for (k = 0; k < Nspace; k++) {
        if (atmos.T[k] >= ar85t1 && atmos.T[k] <= ar85t2) {
          ...
          cdn = ar85a * 1E-9 * pow(t4, ar85b) *
                (1.0 + ar85c * exp(ar85d * t4)) * atmos.H->n[0][k] *
                CUBE(CM_TO_M);
          atom->C[ij][k] += cdn;
        }
      }
      ...
      for (k = 0; k < Nspace; k++) {
        ...
        cup = 2.1715E-8 * cbar * pow(13.6 / de, 1.5) * sqrt(dekt) * E1(dekt) *
              wb * atmos.ne[k] * CUBE(CM_TO_M);
  ...
  // fsetpos(fp_atom, &collpos);  // Tiago: not needed now (but when to free
  // fp_atom?!)
