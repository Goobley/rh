...
double PII(double adamp, double waveRatio, double q_emit, double q_abs) {
  ...
  static double xg[NGAUSS / 2] = {0.183434642495, 0.525532409916,
                                  0.796666477413, 0.960289856497};
  static double wg[NGAUSS / 2] = {0.362683783378, 0.313706645877,
                                  0.222381034453, 0.101228536290};
  static double sn[NGAUSS], cs[NGAUSS];

  if (initialize) {
    for (n = 0; n < NGAUSS; n++) {
      theta =
          (n % 2) ? 0.5 * PI * (1.0 - xg[n / 2]) : 0.5 * PI * (1.0 + xg[n / 2]);
      sn[n] = sin(theta);
      cs[n] = cos(theta);
    }
    initialize = FALSE;
  ...
  for (n = 0; n < NGAUSS; n++) {
    ...
    pii +=
        Voigt(adamp / (a2 * sn[n]), b1 * q_abs + b2 * q_emit, NULL, RYBICKI) *
        exp(-c1 * c1) * wg[n / 2];
  }
  return (0.25 * pii);
}
...
double GII(double adamp, double waveratio, double q_emit, double q_abs) {
  double gii, pcore, aq_emit, umin, epsilon, giiwing, u1, phicore, phiwing;
  ...
  if (q_emit < PRD_QWING) {
    if ((q_abs < -PRD_QWING) || (q_abs > q_emit + waveratio * PRD_QSPREAD))
      return gii;
  ...
  if (q_emit >= PRD_QCORE) {
    ...
    if (q_emit >= PRD_QWING) {
      if (fabs(q_abs - aq_emit) > waveratio * PRD_QSPREAD)
        return gii;
      pcore = 0.0;
    }
    umin = fabs((q_abs - aq_emit) / (1.0 + waveratio));
    giiwing =
        (1.0 + waveratio) * (1.0 - 2.0 * umin * GZERO(umin)) * exp(-SQ(umin));
    ...
    if (waveratio == 1.0) {
      ...
      giiwing -=
          fabs(1.0 - waveratio) * (1.0 - 2.0 * u1 * GZERO(u1)) * exp(-SQ(u1));
    }
