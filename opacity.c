...
double MolProfile(MolecularLine *mrt, int k, int mu, bool_t to_obs,
                  double lambda, double *phi_Q, double *phi_U, double *phi_V,
...
void Opacity(int nspect, int mu, bool_t to_obs, bool_t initialize, int threadId) {
  ...
  double *phi, gijk, twohnu3_c2, twohc, hc_4PI, hc_k, hc, fourPI, *n_i, *n_j,
      Bijxhc_4PI, wlambda, chi_l, *chi_Q, *chi_U, *chi_V, eta_l, *eta_Q, *eta_U,
      *eta_V, *chip_Q, *chip_U, *chip_V, *phi_Q, *phi_U, *phi_V, *psi_Q, *psi_U,
      *psi_V;
  bool_t solveStokes;
  ...
  for (nact = 0; nact < atmos.Nactiveatom; nact++) {
    ...
    for (n = 0; n < as->Nactiveatomrt[nact]; n++) {
      ...
      case ATOMIC_LINE:
        ...
        solveStokes = (line->polarizable && input.StokesMode == FULL_STOKES);
        ...
        if (input.limit_memory) {
          ...
          phi = (double *)malloc(Nrecphi * atmos.Nspace * sizeof(double));
        }
        ...
        if (line->PRD) {
          ...
          case PRD_ANGLE_DEP:
            ...
            for (k = 0; k < atmos.Nspace; k++)
              atom->rhth[nt].gij[n][k] *=
                  line->rho_prd[lamu][k] * input.prdswitch +
                  (1 - input.prdswitch);
            break;
          ...
          case PRD_ANGLE_APPROX:
            ...
            if (input.prdh_limit_mem) {
              ...
              for (k = 0; k < atmos.Nspace; k++) {
                ...
                //  prd factor at constant k, with wavelength contiguous in
                //  memory
                for (lamu = 0; lamu < line->Nlambda; lamu++)
                  rho_tmp[lamu] = line->rho_prd[lamu][k];
                // Interpolate to account for Doppler shift
                Linear(line->Nlambda, line->lambda, &(rho_tmp[0]), 1, &lag,
                       &rho_int, TRUE);
                atom->rhth[nt].gij[n][k] *=
                    rho_int * input.prdswitch + (1 - input.prdswitch);
              }
              ...
              for (k = 0; k < atmos.Nspace; k++) {
                ...
                rho_int =
                    (1.0 - line->frac[lamu][k]) *
                        line->rho_prd[line->id0[lamu][k]][k] +
          ...
          case PRD_ANGLE_INDEP:
            for (k = 0; k < atmos.Nspace; k++) {
              atom->rhth[nt].gij[n][k] *=
                  line->rho_prd[la][k] * input.prdswitch +
                  (1 - input.prdswitch);
            }
            break;
          }
      ...
      if (twohnu3_c2) {
        ...
        if (as->art[nact][n].type == ATOMIC_LINE && solveStokes) {
          ...
          for (k = 0; k < atmos.Nspace; k++) {
            chi_l = Bijxhc_4PI * (n_i[k] - atom->rhth[nt].gij[n][k] * n_j[k]);
            ...
            eta_l = Bijxhc_4PI * twohnu3_c2 * atom->rhth[nt].gij[n][k] * n_j[k];
    ...
    for (n = 0; n < as->Nactivemolrt[nact]; n++) {
      ...
      if (twohnu3_c2) {
        for (k = 0; k < atmos.Nspace; k++) {
          ...
          molecule->rhth[nt].eta[k] += twohnu3_c2 *
                                       molecule->rhth[nt].gij[n][k] *
...
flags MolecularOpacity(double lambda, int nspect, int mu, bool_t to_obs,
  ...
  int NrecStokes;
  double dlamb_char0, dlamb_charN, kT, hc_la, hc, fourPI, hc_4PI, Bijhc_4PI,
      ni_gi, nj_gj, twohnu3_c2, phi, phi_Q, phi_U, phi_V, psi_Q, psi_U, psi_V,
      chi_l, *chi_Q, *chi_U, *chi_V, eta_l, *eta_Q, *eta_U, *eta_V, *chip_Q,
      *chip_U, *chip_V;
  Molecule *molecule;
  MolecularLine *mrt;
  flags backgrflags;
  ...
  for (n = 0; n < atmos.Nmolecule; n++) {
    ...
    if ((molecule->Nrt > 0 && !molecule->active)) {
      dlamb_char0 =
          lambda * molecule->mrt[0].qwing * (atmos.vmicro_char / CLIGHT);
      ...
      if (lambda >= molecule->mrt[0].lambda0 - dlamb_char0 &&
        ...
        for (kr = 0; kr < molecule->Nrt; kr++) {
          ...
          if (fabs(mrt->lambda0 - lambda) <= dlamb_char0) {
            ...
            for (k = 0; k < atmos.Nspace; k++) {
              if (molecule->n[k] > 0.0) {
                phi = MolProfile(mrt, k, mu, to_obs, lambda, &phi_Q, &phi_U,
...
double MolProfile(MolecularLine *mrt, int k, int mu, bool_t to_obs,
  ...
  double v, phi_sm, phi_sp, phi_pi, psi_sm, psi_sp, psi_pi, adamp, vB, H, F, sv,
      phi_sigma, phi_delta, sign, sin2_gamma, phi, psi_sigma, psi_delta;
  ...
  adamp =
      mrt->Aji * (mrt->lambda0 * NM_TO_M) / (4.0 * PI * molecule->vbroad[k]);
