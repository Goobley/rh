...
void readMolecule(Molecule *molecule, char *fileName, bool_t active) {
  ...
  char inputLine[MAX_LINE_SIZE], fitStr[20], line_data[MAX_LINE_SIZE],
      elementID[ATOM_ID_WIDTH + 1], *token, popsFile[MAX_LINE_SIZE],
  ...
  if ((fp_molecule = fopen(fileName, "r")) == NULL) {
    ...
    sprintf(messageStr, " -- reading input file: %s %s", fileName,
            (active) ? "(active)\n\n" : "(passive)\n");
  ...
  molecule->pt_index = (int *)malloc(molecule->Nelement * sizeof(int));
  molecule->pt_count = (int *)malloc(molecule->Nelement * sizeof(int));
  molecule->Nnuclei = 0;
  ...
  for (n = 0; n < molecule->Nelement; n++) {
    ...
    if (!match) {
      sprintf(messageStr,
              "Could not find element %s of molecule %s"
  ...
  if (strstr(fitStr, "KURUCZ_70"))
    ...
    sprintf(messageStr, "Unable to regognize keyword %s in file %s", fitStr,
            fileName);
    Error(ERROR_LEVEL_2, routineName, messageStr);
  }
  ...
  * --- Read minimum and maximum formation temperature -- ---------- */
  ...
  for (n = 0; n < molecule->Nelement; n++)
    molecule->weight +=
        molecule->pt_count[n] * atmos.elements[molecule->pt_index[n]].weight;
  ...
  while (getLine(fp_molecule, COMMENT_CHAR, inputLine, exit_on_EOF = FALSE) !=
         EOF) {
  ...
  if (active) {
    ...
    molecule->nv =
        matrix_double(molecule->Nv * molecule->Nconfig, atmos.Nspace);
    molecule->nvstar =
        matrix_double(molecule->Nv * molecule->Nconfig, atmos.Nspace);
    molecule->pfv =
        matrix_double(molecule->Nv * molecule->Nconfig, atmos.Nspace);
    ...
    molecule->popsFile = (char *)malloc((strlen(popsFile) + 1) * sizeof(char));
  ...
  if (molecule->active) {
    ...
    molecule->rhth = (rhthread *)malloc(input.Nthreads * sizeof(rhthread));
  }

  fclose(fp_molecule);
  sprintf(labelStr, "Read %s %3s", (molecule->active) ? "Active" : "Molecule",
          molecule->ID);
...
void freeMolecule(Molecule *molecule) {
  ...
  if (molecule->pt_index != NULL)
    free(molecule->pt_index);
  if (molecule->pt_count != NULL)
    free(molecule->pt_count);
  if (molecule->vbroad != NULL)
    free(molecule->vbroad);
  if (molecule->pf_coef != NULL)
    free(molecule->pf_coef);
  if (molecule->eqc_coef != NULL)
    free(molecule->eqc_coef);
  if (molecule->pf != NULL)
    free(molecule->pf);
  ...
  if (molecule->C_ul != NULL)
    free(molecule->C_ul);
  if (molecule->Gamma != NULL)
...
void readMolecularLines(struct Molecule *molecule, char *line_data) {
  ...
  Nread = sscanf(inputLine, "%d %s %s", &Nrt, type_string, format_string);
  ...
  molecule->mrt = (MolecularLine *)realloc(
      molecule->mrt, (molecule->Nrt + Nrt) * sizeof(MolecularLine));
  ...
  if (strstr(format_string, "GOORVITCH94")) {
    for (kr = molecule->Nrt; kr < molecule->Nrt + Nrt; kr++) {
      ...
      Nread = sscanf(inputLine, "%lf %lf %lf %lf %lf %lf %d %d %s %d %d",
                     ...
                     &strength, &mrt->vj, &mrt->vi, branchStr, &Ji, &isotope);
      ...
      default:
        sprintf(messageStr,
                "Invalid value of branch stringin file %s: %s\n"
      ...
      mrt->Bji =
          CUBE(mrt->lambda0 * NM_TO_M) / (2.0 * HPLANCK * CLIGHT) * mrt->Aji;
      ...
      case 12:
      case 26:
        mrt->isotope_frac = 1.0 - C_13;
        break;
      case 13:
      case 36:
        mrt->isotope_frac = C_13;
        break;
      default:
    ...
    for (kr = molecule->Nrt; kr < molecule->Nrt + Nrt; kr++) {
      ...
      if (strstr(format_string, "KURUCZ_TIO")) {
        Nread =
            sscanf(inputLine, "%10lf %7lf %5lf %10lf %5lf %10lf", &lambda_air,
                   &log_gf, &mrt->gi, &mrt->Ei, &mrt->gj, &mrt->Ej);
      } else {
        Nread =
            sscanf(inputLine, "%10lf %7lf %5lf %10lf %5lf %11lf", &lambda_air,
                   &log_gf, &mrt->gi, &mrt->Ei, &mrt->gj, &mrt->Ej);
      }
      ...
      if (strstr(format_string, "KURUCZ_CD18")) {
        ...
        switch (isotope) {
        case 29:
          mrt->isotope_frac = SI_29;
          break;
        case 30:
          mrt->isotope_frac = SI_30;
          break;
        default:
          mrt->isotope_frac = 1.0;
        }
        ...
        switch (isotope) {
        case 25:
          mrt->isotope_frac = MG_25;
          break;
        case 26:
          mrt->isotope_frac = MG_26;
          break;
        default:
          mrt->isotope_frac = 1.0;
        }
        ...
        switch (isotope) {
        case 46:
          mrt->isotope_frac = TI_46;
          break;
        case 47:
          mrt->isotope_frac = TI_47;
          break;
        case 48:
          mrt->isotope_frac = TI_48;
          break;
        case 49:
          mrt->isotope_frac = TI_49;
          break;
        case 50:
          mrt->isotope_frac = TI_50;
          break;
        default:
          mrt->isotope_frac = 1.0;
        }
      ...
      if (strlen(inputLine) > 71) {
        ...
        Nread = sscanf(inputLine + 71, "%1s %1s %lf %1s %1s %lf", Hundi,
                       Lambdai, &mrt->Si, Hundj, Lambdaj, &mrt->Sj);
...
void readMolecularModels(void) {
  ...
  if ((fp_molecules = fopen(input.molecules_input, "r")) == NULL) {
    sprintf(messageStr, "Unable to open input file %s", input.molecules_input);
    Error(ERROR_LEVEL_2, routineName, messageStr);
  }
  ...
  atmos.molecules = (Molecule *)malloc(atmos.Nmolecule * sizeof(Molecule));
  ...
  for (n = 0; n < atmos.Nelem; n++) {
    ...
    if (element->Nmolecule == 0) {
      ...
      element->mol_index =
          (int *)realloc(element->mol_index, element->Nmolecule * sizeof(int));
  }
